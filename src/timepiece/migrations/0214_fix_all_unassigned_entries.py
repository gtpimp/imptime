...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for entry in orm['timepiece.Entry'].objects.exclude(project=F('issue__project')):
            issue, is_new = orm['timepiece.Issue'].objects.get_or_create(status='new',
                                                                         project=entry.issue.project,
                                                                         subject='Adhoc%d' % entry.id,
                                                                         defaults={'auto_created_during_import':True,
                                                                                   'adhoc':True,
                                                                                   'assigned_to':entry.user,
                                                                                   'number':Issue.get_next_issue_number(entry.issue.project.business),
                                                                                   'description':'ad-hoc',
                                                                                   'story_points':0,
                                                                                   'order':Issue.get_next_order(entry.issue.project)})
            entry.issue = issue
