...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        businesses = orm['timepiece.Business'].objects.all()
        for business in businesses:
            project_codes = set(x['code'] for x in orm['timepiece.Project'].objects.filter(business=business).values('code'))
            for project_code in project_codes:
                business_projects = orm['timepiece.Project'].objects.filter(code=project_code, business=business)
                primary_project = business_projects[0]
                secondary_projects = business_projects.exclude(pk=primary_project.id)
                #if business_projects.count() > 0: print business_projects.count(), secondary_projects.count()
                for secondary_project in secondary_projects:
                    for entry in orm['timepiece.Entry'].objects.filter(project=secondary_project):
                        entry.project = primary_project
                        entry.save()
                #print "deleting", secondary_projects.count()
                secondary_projects.delete()
