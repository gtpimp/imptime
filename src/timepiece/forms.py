...
from timepiece.lookups import (BusinessLookup, ProjectLookup, QuickLookup,
                               UserLookup)
from timepiece.models import (Activity, Attribute, Business, BusinessComment,
                              BusinessPermissions, CalendarEvent, Company,
                              DevChecklist, Entry, FinanceChecklist, Issue,
                              IssueStatus, Location, Project, ProjectHours,
                              Salary, Schedule, TrafficChecklist, UserProfile)
...
class ProjectFiltersForm(forms.Form):
    ...
    non_billable = forms.BooleanField(
        label="Non-Billable", initial=True, required=False
    )
    paid_leave = forms.BooleanField(initial=True, required=False)
    trunc = forms.ChoiceField(
        label="Group Totals By:",
...
class QuickSearchForm(forms.Form):
    quick_search = selectable_forms.AutoCompleteSelectField(
        QuickLookup, label="Quick Search", required=False
    )
        ...
        if item is not None:
            try:
                ...
                raise forms.ValidationError(
                    "%s" % "User, business, or project does not exist"
                )
        else:
            raise forms.ValidationError(
                "%s" % "User, business, or project does not exist"
    ...
    def save(self):
        ...
        if type == "individual":
            return reverse("view_person", kwargs={"person_id": pk})
        elif type == "business":
            return reverse("view_business", kwargs={"business": pk})
        elif type == "project":
            return reverse("view_project", kwargs={"project_id": pk})
    ...
    def __init__(self, *args, **kwargs):
        ...
        if business is not None:
            self.fields["user"].widget.choices = [("", "")] + [
                (x.id, str(x)) for x in business.users.order_by("username")
            ]
...
class ClockInForm(forms.ModelForm):
    active_comment = forms.CharField(
        label="Notes for the active entry", widget=forms.Textarea, required=False
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        try:
            last_project_entry = timepiece.Entry.objects.filter(
                user=self.user, issue__project=project
            ).order_by("-end_time")[0]
        except IndexError:
...
class ImportEntriesForm(forms.Form):
    raw_entries = forms.CharField(
        max_length=50000, required=False, widget=forms.Textarea
    )
    ...
    def _ignore_project(self, entry):
        ...
        ignore_projects_codes = [
            Project.get_code_from_name(name) for name in ignore_projects_names
        ]
        ...
        for raw_entry in raw_entries.split("\n"):
            ...
            try:
                date = datetime.fromtimestamp(
                    mktime(time.strptime(raw_date, "%d-%b-%y"))
                )
            except ValueError:
                errors.append(
                    {
            ...
            try:
                ...
                try:
                    ...
                    allowed_businesses = BusinessPermissions.active_businesses_for_user(
                        self.user
                    )
                    errors.append(
                        {
                            ...
                            "msg": "Are you assigned to this business? Possible businesses are: %s"
                            % (", ".join([b.name for b in allowed_businesses])),
                        }
                    )
                    continue
            ...
            try:
                ...
                try:
                    project = allowed_projects.get(
                        name=lookup_project(
                            raw_project, (p.name for p in allowed_projects)
                        )
                    )
                    ...
                    errors.append(
                        {
                            ...
                            "msg": "Are you assigned to this sprint? Possible sprints are: %s"
                            % (", ".join([p.name for p in allowed_projects])),
                        }
                    )
                    continue
                    ...
                    errors.append(
                        {
                            ...
                            "msg": "Possible issue numbers are: %s"
                            % (
                                ", ".join(
                                    [
                                        str(i.number)
                                        for i in project.issues.all().order_by("order")
                                    ]
                                )
                            ),
                        }
                ...
                errors.append(
                    {
                        "line": raw_entry,
                        "line_number": line_number,
                        "error": "Invalid hours value %s for sprint name %s."
                        % (raw_hours, raw_project),
                        "msg": "Invalid hours value '%s' for sprint name %s. Please check the value."
                        % (raw_hours, raw_project),
                    }
                )
                continue
            ...
            end_time = datetime(
                ...
                start_time.month,
                start_time.day,
                int(round(start_time.hour + hours)),
                int(round(start_time.minute + minutes)),
            )
...
class AddUpdateEntryForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["project"].choices = (
            ...
            .filter(Q(status__label="open") | Q(status__label="reopened"))
    ...
    def clean(self):
        ...
        start = datetime(start_date.year, start_date.month, start_date.day)
            ...
            start.month,
            start.day,
            int(round(start.hour + hours)),
            int(round(start.minute + minutes)),
        )
        ...
        if not start:
            raise forms.ValidationError("Please enter a valid date/time.")
        ...
        if issue_number.strip():
            try:
                ...
                raise forms.ValidationError(
                    "Please enter a valid issue number", code="issue_number"
                )
    ...
    def save(self, commit=True):
        ...
        if self.cleaned_data["issue_number"].strip():
            self.instance.issue = project.issues.get(
                number=self.cleaned_data["issue_number"].strip()
            )
...
class DateForm(forms.Form):
    ...
    status = forms.ChoiceField(
        choices=STATUS_CHOICES, widget=forms.HiddenInput(), required=False
    )
    ...
    def clean(self):
        ...
        if (
            data["from_date"]
            and data["to_date"]
            and data["from_date"] > data["to_date"]
        ):
...
class YearMonthForm(forms.Form):
    MONTH_CHOICES = [
        (i, time.strftime("%B", time.strptime(str(i), "%m"))) for i in range(1, 13)
    ]
    ...
    def __init__(self, *args, **kwargs):
        ...
        try:
            first_entry = timepiece.Entry.objects_original.values("end_time").order_by(
                "end_time"
            )[0]
        except IndexError:
    ...
    def save(self):
        ...
        if (
            "select_all" in self.cleaned_data
            and self.cleaned_data.get("select_all") == "all"
        ):
            return (None, None)
        else:
            return (from_date, to_date)
...
class UserYearMonthForm(YearMonthForm):
    users = auth_models.User.objects.exclude(timepiece_entries=None).order_by(
        "first_name"
    )
...
class IssueStatusForm(forms.ModelForm):
    ...
    def __init__(self, business, *args, **kwargs):
        ...
        self.fields["status2"].queryset = IssueStatus.objects.filter(
            business=business
        ).order_by("name")
...
class IssueForm(forms.ModelForm):
    ...
    class Meta:
        model = timepiece.Issue
        fields = ("subject", "description", "status2", "assigned_to")
    ...
    def __init__(self, *args, **kwargs):
        ...
        if business is not None:
            self.fields["status2"].initial = IssueStatus.objects.get_or_create(
                business=business, name="new"
            )[0]
            self.fields["assigned_to"].widget.choices = [("", "")] + [
                (x.id, str(x)) for x in business.users
            ]
...
class UserProfileForm(forms.ModelForm):
    groups = forms.ModelMultipleChoiceField(
        queryset=auth_models.Group.objects.all(), widget=CheckboxSelectMultiple
    )
...
class ProjectSearchForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        PROJ_STATUS_CHOICES.extend(
            (a.pk, a.label)
            for a in Attribute.objects.all().filter(type="project-status")
        )
...
class AggregatedTimesheetFormByProject(forms.Form):
    ...
    project = forms.ModelChoiceField(
        label="Project:", queryset=Project.objects.order_by("business__name", "name")
    )
    ...
    def __init__(self, user, *args, **kwargs):
        ...
        self.fields["project"].queryset = Project.objects.filter(users=user).order_by(
            "business__name", "name"
        )
...
class GraphFilterForm(forms.Form):
    ...
    enabled_series = forms.MultipleChoiceField(
        required=False,
        label="Graphs",
        choices=(
            ("all_hours", "all hours"),
    ...
    def __init__(self, user, *args, **kwargs):
        ...
        self.fields["business"].queryset = (
            BusinessPermissions.active_businesses_for_user(user).order_by(
                "name", "name"
            )
        )
...
def createIssueStatusFormset(business):
    formset = modelformset_factory(
        timepiece.Issue, form=IssueStatusForm, extra=0, can_delete=True, exclude=[]
    )
    formset.form = staticmethod(curry(IssueStatusForm, business=business))
    return formset


expense_formset = modelformset_factory(
    timepiece.Expense, can_delete=True, extra=2, exclude=[]
)
permissions_formset = modelformset_factory(
    timepiece.BusinessPermissions,
    form=EditPersonPermission,
    extra=0,
...
rate_formset = modelformset_factory(
    timepiece.UserProfile, form=RateForm, can_delete=False, extra=0
)
...
class SprintInvoiceReportSettingsForm(forms.Form):
    ...
    include_billable_per_user = forms.BooleanField(
        label="Tick to include billable per user", initial=True, required=False
    )
    only_these_statuses = forms.MultipleChoiceField(
        label="Only include these statuses",
        required=True,
        initial=("all",),
        widget=CheckboxSelectMultiple,
    )
    only_assigned_to = forms.MultipleChoiceField(
        label="Only assigned to these users",
        initial=("all",),
        required=False,
        widget=CheckboxSelectMultiple,
    )

    only_these_issue_numbers = forms.MultipleChoiceField(
        required=False, widget=CheckboxSelectMultiple
    )
    include_adhoc_issues = forms.BooleanField(
        label="Include Adhoc issues", initial=False, required=False
    )
    ...
    def __init__(self, project, bp, only_these_issues=None, *args, **kwargs):
        ...
        if project:
            self.fields["only_these_statuses"].choices = [
                ("all", "Any status"),
            ] + list(
                [
                    (x["id"], x["status2__name"])
                    for x in project.issues.values("id", "status2__name").distinct()
                ]
            )
            self.fields["only_assigned_to"].choices = [
                ("all", "Any user"),
            ] + list(
                [
                    (x["assigned_to__username"], x["assigned_to__username"])
                    for x in project.issues.exclude(assigned_to__isnull=True)
        ...
        if only_these_issues is None:
            if project:
                ...
                only_these_issues = []
        self.fields["only_these_issue_numbers"].choices = [
            (issue.number, issue.number) for issue in only_these_issues
        ]
        self.fields["only_these_issue_numbers"].initial = [
            issue.number for issue in only_these_issues
        ]
...
class SprintQuoteReportSettingsForm(forms.Form):
    ...
    preamble_type = forms.ChoiceField(
        label="Preamble type",
        required=False,
        choices=(("billable", "Estimates"), ("quote", "Quote")),
    )
    ...
    include_rates = forms.BooleanField(
        label="Tick to include rates", initial=False, required=False
    )
    show_hours = forms.BooleanField(
        label="Tick to show hours", initial=False, required=False
    )
    show_breakdown = forms.BooleanField(
        label="Tick to show breakdown", initial=False, required=False
    )
    show_billable = forms.BooleanField(
        label="Tick to show billable cost for the sprint", initial=True, required=False
    )
    show_issue_billable = forms.BooleanField(
        label="Tick to show billable cost per item", initial=True, required=False
    )

    preferred_user_for_estimates = forms.ChoiceField(
        label="User's estimates to use where conflicts", required=False
    )

    include_exclude_vat = forms.ChoiceField(
        label="Include or Exclude VAT in quote",
        required=True,
        choices=(
            ("exclude_vat", "Costs exclude VAT"),
    ...
    is_final = forms.BooleanField(
        label="Tick for final, untick for provisional", initial=False, required=False
    )
    ...
    def __init__(self, project, bp, only_these_issues=None, *args, **kwargs):
        ...
        self.fields["preferred_user_for_estimates"].choices = [
            (x.user.id, x.user)
            for x in BusinessPermissions.by_user(project.business).values()
            if x.has_estimate_own_points
        ]
...
class EditBusinessDocumentForm(forms.ModelForm):
    ...
    project = GroupedModelChoiceField(
        "business",
        required=False,
        queryset=timepiece.Project.objects.all()
        .filter_open()
...
class GenerateBusinessDocumentForm(forms.Form):
    ...
    doc_type = forms.ChoiceField(
        required=True, choices=timepiece.BusinessDocument.DOC_TYPE_CHOICES
    )
...
class IssueCheckboxContextMenuChangeStateForm(forms.Form):
    ...
    status = forms.ChoiceField(label="New status", required=True, initial=("New",))
    ...
    def __init__(self, project, *args, **kwargs):
        ...
        self.fields["status"].choices = [
            ...
            [
                (x.id, x.name)
                for x in IssueStatus.objects.filter(business=project.business).order_by(
                    "name"
                )
            ]
        )
...
class IssueCheckboxContextMenuSelectByStateForm(forms.Form):
    ...
    status = forms.ChoiceField(
        label="Status to select", required=True, initial=("New",)
    )

    def __init__(self, project, *args, **kwargs):
        super(IssueCheckboxContextMenuSelectByStateForm, self).__init__(*args, **kwargs)
        statuses_in_use = IssueStatus.objects.filter(
            business=project.business
        ).order_by("name")
        self.fields["status"].choices = [
            ("na", ""),
        ] + list([(x.id, x.name) for x in statuses_in_use])
        self.fields["status"].widget.attrs["onchange"] = "this.form.submit();"


class IssueCheckboxContextMenuChangeAssigneeForm(forms.Form):

    assignee = forms.ChoiceField(label="New assignee", required=True, initial=("New",))

    def __init__(self, project, *args, **kwargs):
        super(IssueCheckboxContextMenuChangeAssigneeForm, self).__init__(
            *args, **kwargs
        )
        self.fields["assignee"].choices = [("", "")] + [
            (x.id, str(x)) for x in project.business.users
        ]
...
class IssueCheckboxContextMenuActiveIssueForm(forms.Form):
    ...
    def __init__(self, project, label, *args, **kwargs):
        ...
        self.fields["focus_issue"].choices = [("", "")] + [
            (x.id, "%s %s" % (x.number, x.subject))
            for x in project.issues.all().order_by_project_id(project.id)
        ]
...
class IssueCheckboxContextMenuChangeIssueAdhocForm(forms.Form):
    ...
    adhoc = forms.ChoiceField(label="Adhoc", required=True, initial=("New",))

    def __init__(self, project, label, *args, **kwargs):
        super(IssueCheckboxContextMenuChangeIssueAdhocForm, self).__init__(
            *args, **kwargs
        )
        self.fields["adhoc"].choices = [
            ("", ""),
            ("set_adhoc", "Make adhoc"),
            ("unset_adhoc", "Make standard issue"),
        ]
...
class CalendarFilterForm(forms.Form):
    ...
    actual_time_aggregation_mechanism = forms.ChoiceField(
        required=False,
        initial="none",
        choices=(
            ("none", "individual events"),
            ("by_project", "Total per project"),
    ...
    def save(self, calendar_events, entry_events, holiday_events):
        ...
        if len(self.cleaned_data["businesses"]) > 0:
            calendar_events = calendar_events.filter(
                Q(project__business__in=self.cleaned_data["businesses"])
                | Q(project__isnull=True)
            )
            entry_events = entry_events.filter(
                project__business__in=self.cleaned_data["businesses"]
            )
        if self.cleaned_data["start"] is not None:
            calendar_events = calendar_events.filter(
                start__gte=self.cleaned_data["start"]
            )
            entry_events = entry_events.filter(
                start_time__gte=self.cleaned_data["start"]
            )
            holiday_events = holiday_events.filter(
                applies_on__gte=self.cleaned_data["start"]
            )
        if self.cleaned_data["end"] is not None:
            calendar_events = calendar_events.filter(
                start__lte=self.cleaned_data["end"]
            )
            entry_events = entry_events.filter(end_time__lte=self.cleaned_data["end"])
            holiday_events = holiday_events.filter(
                applies_on__lte=self.cleaned_data["end"]
            )
        if (
            self.cleaned_data["event_types"] is not None
            and len(self.cleaned_data["event_types"]) > 0
        ):
            calendar_events = calendar_events.filter(
                event_type__in=self.cleaned_data["event_types"]
            )

        if (
            self.cleaned_data["event_types"] is None
            or "actual" not in self.cleaned_data["event_types"]
        ):
            entry_events = []
        else:
            ...
            if self.cleaned_data["actual_time_aggregation_mechanism"] == "by_project":
                entry_events = (
                    entry_events.extra(select={"day": "date( start_time )"})
                    .values("user", "project__business", "day")
                    .annotate(Sum("hours"))
                )
...
class CalendarEventCreateForm(forms.ModelForm):
    ...
    status = forms.ChoiceField(
        required=False, initial="CONFIRMED", choices=(CalendarEvent.EVENT_STATUSES)
    )
...
class CalendarEventUpdateForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        if (
            "event_type" not in data
            or data["event_type"] is None
            or len(data["event_type"]) == 0
        ):
...
class QuickClockerForm(forms.Form):
    ...
    project = forms.ModelChoiceField(
        required=True, queryset=timepiece.Project.objects.none()
    )
    ...
    def __init__(self, logged_in_user, users, projects, *args, **kwargs):
        ...
        self.fields["project"].choices = [
            (
                x.id,
                "<b>%s</b>     %s <span searchable_name='%s' style='font-size:9px'>(%s)</span>"
                % (
                    x.business.name,
                    x.name,
                    (x.name.lower() + " " + x.business.name.lower()),
                    x.status3.name,
                ),
            )
            for x in projects
...
class QuickClockerClockOutForm(forms.Form):
    entry = forms.ModelChoiceField(
        required=True, queryset=Entry.objects.none(), label=""
    )
    ...
    def __init__(self, entries, *args, **kwargs):
        ...
        self.fields["entry"].choices = [
            (
                x.id,
                "<b>%s</b> issue%s <b>%s</b> %s"
                % (
                    x.user.username,
                    x.issue.number,
                    x.issue.project.long_name(),
                    x.start_time.strftime("%a %H:%M"),
                ),
            )
            for x in entries
        ]
...
class QuickClockerEditEntry(forms.ModelForm):
    ...
    issue = GroupedModelChoiceField(
        "issue", required=False, queryset=timepiece.Issue.objects.none()
    )
    project = GroupedModelChoiceField(
        "business", required=False, queryset=timepiece.Project.objects.none()
    )
    ...
    def __init__(self, projects, *args, **kwargs):
        ...
        if self.instance and self.instance.project:
            issues = self.instance.project.issues.all().order_by_project_id(
                self.instance.project.id
            )
            self.fields["issue"].queryset = issues
            self.fields["issue"].choices = [
                (None, "default"),
            ] + [(x.id, "issue%d: %s" % (x.number, x.subject)) for x in issues]
