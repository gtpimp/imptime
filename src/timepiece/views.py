...
def traffic_checklist(request, project_id, template="timepiece/project/traffic_checklist.html", context=None):
    ...
    context['form'] = form
    context['project'] = project
    context['previous_checklists'] = timepiece.TrafficChecklist.objects.all().filter(project__in=timepiece.Project.objects.filter(business=project.business)).order_by("-pk")[:5]
    return render_to_response(template, context, context_instance=RequestContext(request))
