...
from invoicing.models import ClientInvoiceDetails, Invoice, Quote
...
from slideshow.views import calculate_dev_hours_stats, calculate_progress_ratio
...
from timepiece.templatetags.timepiece_tags import (get_active_hours,
                                                   seconds_to_hours)
from timepiece.utils import (get_week_start, percentage, render_with,
                             reverse_lazy)
...
def view_entries(request):
    ...
    entries = (
        timepiece.Entry.objects.select_related(
            "issue__project__business",
        )
        .filter(time_q, user=request.user)
    ...
    others_active_entries = (
        timepiece.Entry.objects.filter(
    ...
    my_active_entries = (
        ...
        .only("user", "issue__project", "activity", "start_time")
        .filter(
            user=request.user,
            end_time__isnull=True,
        )
    ...
    project_entries = (
        entries.exclude(issue__project__in=allocated_projects, end_time__isnull=True)
        .values(
    ...
    this_weeks_entries = entries.order_by("-start_time").filter(
...
def clock_in(request):
    ...
    active_entry = timepiece.Entry.objects_original.filter(
        user=request.user, end_time__isnull=True
    )
    # Should never happen, but just in case.
    if len(active_entry) > 1:
        err_msg = (
            "You have more than one active entry and must clock out "
    ...
    form = timepiece_forms.ClockInForm(
        request.POST or None, initial=initial, user=request.user, active=active_entry
    )
    if form.is_valid():
        entry = form.save()
...
def clock_out(request, entry_id):
    ...
    return render(request, "timepiece/time-sheet/entry/clock_out.html", context)
...
def toggle_paused(request, entry_id):
    ...
    try:
        ...
        entry = timepiece.Entry.objects_original.get(
            pk=entry_id, user=request.user, end_time__isnull=True
        )
    except:
...
def create_edit_entry(request, entry_id=None):
    if entry_id:
        try:
            entry = timepiece.Entry.objects_original.get(pk=entry_id, user=request.user)
...
def reject_entry(request, entry_id):
    ...
    return render(
        request,
        "timepiece/time-sheet/entry/reject_entry.html",
        {
            "entry": entry,
...
def delete_entry(request, entry_id):
    ...
    return render(
        request, "timepiece/time-sheet/entry/delete_entry.html", {"entry": entry}
    )
...
def summary(request, username=None):
    ...
    form = timepiece_forms.YearMonthForm(
        request.GET or None, initial={"month": from_date.month, "year": from_date.year}
    )
    ...
    entries = (
        timepiece.Entry.objects.filter_by_logged_in_user(request.user)
        .values(
            "issue__project__id",
    ...
    people_totals = timepiece.Entry.objects_original.filter_by_logged_in_user(
        request.user
    ...
    business_per_person_totals = (
        entries.filter(dates)
        .values("user__username", "issue__project__business__name")
        .annotate(total_hours=Sum("hours"))
        .order_by("issue__project__business__name")
    )
    business_per_project_per_person_totals = (
        entries.filter(dates)
        .values(
            "user__username", "issue__project__name", "issue__project__business__name"
        )
        .annotate(total_hours=Sum("hours"))
        .order_by("issue__project__name")
    )
...
def view_summary(request, user_id, include_older_businesses=False):
    ...
    context = {
        "current_businesses": current_businesses,
        "old_businesses": old_businesses,
    }
    return render(request, "timepiece/time-sheet/people/projects.html", context)
...
def get_project_card_for_business(request, business_id, project_id):
    ...
    context = {
        ...
        "project": project,
        "quotes": Quote.objects.filter(project=project).order_by(
            "accepted_at", "sent_to_client_at"
        ),
        "invoices": Invoice.objects.filter_by_logged_in_user(request.user)
        .filter(project=project)
        .order_by("invoice_number"),
        "business_permissions_by_user": timepiece.BusinessPermissions.by_user(business),
    }
    return render(request, "timepiece/project/card.html", context)
...
def view_person_time_sheet(request, user_id):
    ...
    to_date = from_date + relativedelta(months=1)
    can_view_summary = request.user and request.user.has_perm(
        "timepiece.view_entry_summary"
    )
    form = (
        timepiece_forms.UserYearMonthForm
        if can_view_summary
        else timepiece_forms.YearMonthForm
    )
    ...
    project_entries = (
        month_qs.order_by()
        .values("issue__project__name", "issue__project__business__name")
        .annotate(sum=Sum("hours"))
        .order_by("-sum")
    )
    ...
    return render(request, "timepiece/time-sheet/people/view.html", context)
...
def change_person_time_sheet(request, action, user_id, from_date):
    ...
    if not perm:
        return HttpResponseForbidden(
            "Forbidden: You cannot {0} this " "timesheet".format(action)
        )
    ...
    try:
        ...
        raise Http404
    to_date = from_date + relativedelta(months=1)
    entries = timepiece.Entry.objects_original.filter(
        user=user_id, end_time__gte=from_date, end_time__lt=to_date
    )
    active_entries = timepiece.Entry.objects_original.filter(
        user=user_id, start_time__lt=to_date, end_time=None, status="unverified"
    )
    filter_status = {
        "verify": "unverified",
        "approve": "verified",
    ...
    if active_entries:
        msg = (
            "You cannot verify/approve this timesheet while the user {0} "
    ...
    return render(request, "timepiece/time-sheet/people/change_status.html", context)
...
def confirm_invoice_project(request, project_id, to_date, from_date=None):
    if not request.user.has_perm("timepiece.generate_project_invoice"):
        return HttpResponseForbidden("Forbidden")
    try:
        to_date = utils.add_timezone(datetime.datetime.strptime(to_date, "%Y-%m-%d"))
        if from_date:
            from_date = utils.add_timezone(
                datetime.datetime.strptime(from_date, "%Y-%m-%d")
            )
    ...
    invoice_form = timepiece_forms.InvoiceForm(request.POST or None, initial=initial)
    if request.POST and invoice_form.is_valid():
        invoice = invoice_form.save()
        entries = timepiece.Entry.objects_original.filter_by_logged_in_user(
            request.user
        ).filter(**entries_query)
        entries.update(status=invoice.status, entry_group=invoice)
        return HttpResponseRedirect(reverse("view_invoice", args=[invoice.pk]))
    else:
        entries = timepiece.Entry.objects.filter_by_logged_in_user(request.user).filter(
            **entries_query
        )
...
def invoice_projects(request):
    ...
    project_totals = (
        entries.filter(
            status="approved",
            issue__project__type__billable=True,
            issue__project__status__billable=True,
        )
        .values(
            "issue__project__type__pk",
            "issue__project__type__label",
            "issue__project__name",
            "hours",
            "issue__project__pk",
            "status",
            "issue__project__status__label",
            "issue__project__business__name",
        )
        .annotate(s=Sum("hours"))
        .order_by("issue__project__type__label", "issue__project__name", "status")
    )
    return render(
        request,
...
class InvoiceEdit(InvoiceDetail):
    ...
    def post(self, request, **kwargs):
        ...
        invoice_form = timepiece_forms.InvoiceForm(
            request.POST, initial=initial, instance=invoice
        )
...
def remove_invoice_entry(request, invoice_id, entry_id):
    ...
    if request.POST:
        ...
        return render(
            request, "timepiece/time-sheet/invoice/remove_invoice_entry.html", context
        )
...
def list_businesses(request):
    ...
    if form.is_valid() and "search" in request.GET:
        ...
        if businesses.count() == 1:
            ...
            return HttpResponseRedirect(reverse("view_business", kwargs=url_kwargs))
    else:
...
def create_edit_business(request, business=None):
    ...
    if request.POST:
        ...
        if business_form.is_valid():
            ...
            _set_project_rate_to_default_for_user(
                request.user, business.sprints.first()
            )
            return HttpResponseRedirect(
                reverse("closed_project_list", kwargs={"business_id": business.id})
            )
    else:
        business_form = timepiece_forms.BusinessForm(instance=business)
...
def list_people(request, template="timepiece/person/list.html"):
    ...
    if not request.user.is_superuser:
        people = people.filter(
            profile__impd_client_id=request.user.profile.impd_client_id
        )
...
def edit_leave(request, person_id=None, template="timepiece/person/edit_leave.html"):
    ...
    if form.is_valid():
        timepiece.CalendarEvent.objects.get_or_create(
    ...
    context["existing_leaves"] = timepiece.CalendarEvent.objects.filter(
        user=person
    ).order_by("-start")
...
def create_edit_person(
    request, person_id=None, template="timepiece/person/create_edit.html"
):
    ...
    if request.POST:
        if person:
            ...
            profile_form = timepiece_forms.UserProfileForm(
                creator=request.user, data=request.POST, prefix="profile"
            )
        ...
        if person:
            ...
            profile_form = timepiece_forms.UserProfileForm(
                request.user, instance=profile, prefix="profile"
            )
            person_form = timepiece_forms.EditPersonForm(instance=person)
        else:
            profile_form = timepiece_forms.UserProfileForm(
                request.user, prefix="profile"
            )
            person_form = timepiece_forms.CreatePersonForm()
...
def project_detail(request, business_id):
    if request.GET:
        ...
        form = timepiece_forms.ProjectSearchForm({"status": "5"})
    ...
    has_edit_project_detail = timepiece.BusinessPermissions.objects.get_or_create(
        business=business, user=request.user
    )[0].has_edit_project_detail
    ...
    if form.is_valid():
        ...
        projects = timepiece.Project.objects.filter_by_logged_in_user(
            request.user
        ).filter(status__label="open")
    ...
    if request.GET:
        if from_date and to_date:
            projects = projects.filter(
                entries__start_time__range=(from_date, to_date)
            ).distinct()
        elif from_date:
    ...
    projects = projects.annotate(
        end_time=Max("entries__end_time"), start_time=Min("entries__start_time")
    )
    ...
    businesses = dict(
        (b, _business_total(p, from_date, to_date)) for b, p in businesses.iteritems()
    )
    user_totals = {}
    ...
    for user in User.objects.all().distinct():
        last_active[user.username] = entries.filter(user=user).aggregate(
    ...
    if "selected_issue_ids_for_context_menu" in request.session:
        context["selected_issue_ids"] = [
            int(x) for x in request.session["selected_issue_ids_for_context_menu"]
        ]
...
def amounts_billed(request):
    ...
    projects = projects.annotate(
        end_time=Max("issues__entries__end_time"),
        start_time=Min("issues__entries__start_time"),
    )
...
def list_projects(request):
    ...
    businesses = timepiece.Business.get_related_business_by_user(request.user).order_by(
        "name"
    )

    context = {
        "active_businesses": businesses.filter_has_any_active_projects(),
...
def _sum_user_totals(users_and_hours, totals=None):
    ...
    for project, users in users_and_hours:
        for username, project_totals in users["users"].iteritems():
            ...
            _business_totals["ctc_rate"] = float(_business_totals["revenue"]) / (
                float(_business_totals["hours"]) or 1
            )
            _business_totals["billed_rate"] = float(_business_totals["billed"]) / (
                float(_business_totals["hours"]) or 1
            )
    return totals
...
def _business_total(projects, start_time=None, end_time=None):
    ...
    for project in projects:
        ...
        expense = expense_objects.filter(project=project).aggregate(
            amount=Sum("amount")
        )
        expense_amount = expense["amount"] if expense and expense["amount"] else 0

        invoice = invoice_objects.filter(project=project).aggregate(
            amount=Sum("items__total_cost")
        )
    ...
    return {
        ...
        "projects": projects,
        "hours": hours,
        "users_and_hours": sorted(
            users_and_hours.iteritems(),
            key=lambda p: (
                -time.mktime(p[0].end_time.timetuple())
                if p[0].end_time is not None
                else 0
            ),
        ),
        "expenses": expenses,
...
def view_project(request, project_id):
    ...
    has_edit_project_detail = timepiece.BusinessPermissions.objects.get_or_create(
        business=project.business, user=request.user
    )[0].has_edit_project_detail
    if not has_edit_project_detail:
        raise PermissionDenied
...
def add_user_to_project(request, project_id):
    ...
    if request.POST:
        ...
        if form.is_valid():
            ...
            #     project=project,
            # )
            bp = timepiece.BusinessPermissions.objects.get_or_create(
...
def edit_project_relationship(request, project_id, user_id):
    ...
    if request.POST:
        ...
        relationship_form = timepiece_forms.ProjectRelationshipForm(instance=rel)
...
def invoiced_project(request, project_id=None):
    project = get_object_or_404(timepiece.Project, pk=project_id)
    project.status3 = timepiece.ProjectStatus.objects.get_or_create(
        name="closed", business=project.business
    )[0]
    project.billable = True
    project.save()
    timepiece.Entry.objects.filter_by_logged_in_user(request.user).filter(
...
def update_project(request, project_id=None, template="timepiece/project/edit.html"):
    ...
    if bp.has_edit_project_detail:
        form = timepiece_forms.ProjectForm(
            bp=bp, data=request.POST or None, instance=project
        )
        if request.POST and form.is_valid():
            project = form.save()
            project.save()
...
def edit_project_budget(request, project_id=None):
    project = (
        get_object_or_404(timepiece.Project, pk=project_id) if project_id else None
    )

    has_edit_budget = timepiece.BusinessPermissions.objects.get_or_create(
        business=project.business, user=request.user
    )[0].has_edit_budget
...
def edit_project_deadlines(request, project_id=None):
    ...
    has_edit_deadlines = timepiece.BusinessPermissions.objects.get_or_create(
        business=project.business, user=request.user
    )[0].has_edit_deadlines
...
def payroll_summary(request):
    ...
    year_month_form = timepiece_forms.YearMonthForm(
        request.GET or None, initial={"month": from_date.month, "year": from_date.year}
    )
    ...
    # Weekly totals
    week_entries = timepiece.Entry.objects.filter_by_logged_in_user(
        request.user
    ).date_trunc("week")
    week_entries = week_entries.filter(weekQ, statusQ, workQ)
    date_headers = utils.generate_dates(from_date, last_billable, by="week")
    weekly_totals = list(
        utils.project_totals(week_entries, date_headers, "total", overtime=True)
    )
    # Monthly totals
    leave = (
        timepiece.Entry.objects.filter_by_logged_in_user(request.user)
    ...
    month_entries = timepiece.Entry.objects.filter_by_logged_in_user(
        request.user
    ...
    unverified = entries.filter(monthQ, status="unverified", user__is_active=True)
...
def edit_settings(request):
    ...
    if request.POST:
        ...
        profile_form = timepiece_forms.UserProfileForm(
            request.user, request.POST, instance=profile
        )
        ...
        profile_form = timepiece_forms.UserProfileForm(
            creator=request.user, instance=profile
        )
...
class ContractList(ListView):
    ...
    queryset = (
        timepiece.ProjectContract.objects.filter(status="current")
...
class DeleteProjectView(DeleteView):
    ...
    permissions = (
        "timepiece.add_project",
        "timepiece.change_project",
    )
...
class ReportMixin(object):
    ...
    def get_context_data(self, **kwargs):
        ...
        from_date, to_date = utils.process_dates(
            self.request.GET, end - relativedelta(months=1), end
        )
        ...
        query = Q(end_time__gt=utils.get_week_start(from_date), end_time__lt=to_date)
        ...
        entries = (
            timepiece.Entry.objects.filter_by_logged_in_user(user)
            .date_trunc(
                trunc, extra_values=("activity", "issue__project__status2__name")
            )
            .filter(query)
        )
...
class HourlyReport(ReportMixin, CSVMixin, TemplateView):
    ...
    def get_filename(self, context):
        ...
        return "hours_{0}_to_{1}_by_{2}.csv".format(
            from_date, to_date, context["trunc"]
        )
    ...
    def get_context_data(self, **kwargs):
        ...
        project_totals = (
            utils.project_totals(entries, date_headers, hour_type, total_column=True)
            if entries
            else ""
        )
...
class BillableHours(ReportMixin, TemplateView):
    ...
    def get_hours_data(self, data, entries, date_headers):
        users = data.get("people", "") or entries.order_by("user").values_list(
            "user", flat=True
        )

        activities = data.get(
            "activities", ""
        ) or timepiece.Activity.objects.values_list("pk", flat=True)

        types = data.get(
            "project_types", ""
        ) or timepiece.Attribute.objects.values_list("pk", flat=True)

        filtered_entries = entries.filter(
            user__in=users, activity__in=activities, project__status__in=types
        )

        project_data = utils.project_totals(
            filtered_entries, date_headers, total_column=False
        )
    ...
    def get_context_data(self, **kwargs):
        ...
        form = timepiece_forms.BillableHoursForm(
            self.request.GET or None, choices={"people": people}
        )

        form_data = form.save() if form.is_valid() else {}
        dates, hours_data = self.get_hours_data(form_data, entries, date_headers)
...
class ProjectHoursMixin(object):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        self.week_start = (
            default_week
            if week_start_str == ""
            else utils.get_week_start(
                datetime.datetime.strptime(week_start_str, "%Y-%m-%d").date()
            )
        )

        return super(ProjectHoursMixin, self).dispatch(request, *args, **kwargs)
...
class ProjectHoursAjaxView(ProjectHoursMixin, View):
    ...
    def get_instance(self, data, week_start):
        try:
            ...
            ph = timepiece.ProjectHours.objects.get(
                user=user, project=project, week_start=week
            )
    ...
    def get(self, request, *args, **kwargs):
        ...
        projects = (
            timepiece.Project.objects.filter_by_logged_in_user(request.user)
            .filter(pk__in=inner_qs)
            .values()
            .order_by("name")
        )
        if not request.user.is_superuser:
        ...
        all_users = auth_models.User.objects.filter(groups__permissions=perm).values(
        ...
        return HttpResponse(
            json.dumps(data, cls=DecimalEncoder), content_type="application/json"
        )
    ...
    def duplicate_entries(self, duplicate, week_update):
        ...
        def duplicate_helper():
            try:
                try:
                    bulk_create = getattr(timepiece.ProjectHours.objects, "bulk_create")
        ...
        if week_qs.exists():
            ...
            if inner_qs.exists():
                for ph in inner_qs:
                    prev_ph = prev_week_qs.get(project=ph.project, user=ph.user)
    ...
    def update_week(self, week_start):
        try:
            ...
            msg = "Parameter week_start must be a date in the format " "yyyy-mm-dd"
            return HttpResponse(msg, status=500)

        form = timepiece_forms.ProjectHoursForm(self.request.POST, instance=instance)
...
@login_required
def salary_edit(
    request, user_id, template="timepiece/salary/payslip.html", context=None
):
    context = context or {}
    ...
    date = timezone.now() - relativedelta(months=1)
    from_date = utils.get_month_start(date).date()
    date_form = timepiece_forms.YearMonthForm(
        request.GET or None,
    ...
    context["msg"] = ""
    try:
        salary = timepiece.Salary.objects.get(
            user=user, date__year=from_date.year, date__month=from_date.month
        )
        ...
        if request.POST:
            try:
                ...
                context["msg"] = "(copied from %s)" % previous_salary.date.strftime(
                    "%b%Y"
                )
            except IndexError:
                salary = timepiece.Salary.objects.create(user=user, date=from_date)
    ...
    user_form = timepiece_forms.QuickEditPersonForm(
        request.POST or None, instance=salary.user, prefix="user_form"
    )

    if request.POST and was_locked:
        messages.error(request, "Save failed, the payslip is locked")
        return HttpResponseRedirect(
            reverse("salary_edit", kwargs={"user_id": salary.user.id})
        )
...
def salary_payslip(
    ...
    salary_id,
    preview=True,
    template="timepiece/salary/payslip_pdf.html",
    context=None,
):
    context = context or {}
...
@permission_required("timepiece.view_entry_summary")
@login_required
def incremental_timesheets_by_project(
    request,
    template="timepiece/time-sheet/redmine/incremental_timesheets_by_project.html",
    context=None,
):
    context = context or {}
    form = timepiece_forms.AggregatedTimesheetFormByProject(
    ...
    if form.is_valid():
        ...
        response["Content-Disposition"] = (
            'attachment; filename="%s.csv"' % report_args["project"].name
        )
        return response
...
@login_required
def revenue(
    request, template="timepiece/time-sheet/reports/revenue.html", context=None
):
    context = context or {}
    ...
    entries = (
        entries.values("month", "project", "user")
        .annotate(Sum("hours"))
        .order_by("month")
    )
...
@login_required
def daily_graph(
    request, user_id, template="timepiece/graphs/daily_graph.html", context=None
):
    ...
    from_date, to_date = _get_filter_dates_only(
        request, context, (today - relativedelta(months=1), today)
    )
    ...
    for user in users:
        ...
        daily, weekly, daily_by_project = _get_daily_hours(
            user, entries, from_date, to_date
        )
        ...
        events_in_range = timepiece.CalendarEvent.objects.filter(
            start__gte=from_date, start__lte=to_date
        ).values("start", "hours")
        user_events = events_in_range.filter(user=user)
        daily_hours[user.username]["sick_days"] = user_events.filter(
            event_type="sickday"
        )
        daily_hours[user.username]["leave_days"] = user_events.filter(
            event_type="leave"
        )
        daily_hours[user.username]["office_closed"] = user_events.filter(
            event_type="office_closed"
        )
        daily_hours[user.username]["public_holidays"] = (
            timepiece.Holiday.objects.filter(
                applies_on__gte=from_date, applies_on__lte=to_date
            )
        )
...
def graphs(request, template="timepiece/graphs/graph.html", context=None):
    ...
    if form.is_valid() and "billable_hours" in form.cleaned_data["enabled_series"]:
        series.append(
            _create_billable_hours_series_for_graphs(request, entries, context)
        )
    if form.is_valid() and "expected_hours" in form.cleaned_data["enabled_series"]:
        series.append(
            _create_expected_hours_series_for_graphs(request, entries, context)
        )
    ...
    if (
        form.is_valid()
        and "cash_flow_atrate_with_expenses" in form.cleaned_data["enabled_series"]
    ):
        series.append(
            _create_cash_flow_atrate_with_expenses_series_for_graphs(
                request, entries, context
            )
        )
    if form.is_valid() and "cash_flow_atrate" in form.cleaned_data["enabled_series"]:
        series.append(
            _create_cash_flow_atrate_series_for_graphs(request, entries, context)
        )
    if form.is_valid() and "cash_flow_invoiced" in form.cleaned_data["enabled_series"]:
        series.append(
            _create_cash_flow_invoiced_series_for_graphs(request, entries, context)
        )
...
def _get_daily_hours(user, entries, from_date=None, to_date=None):
    ...
    entries_hours_per_day = (
        entries.values("on_day").order_by("on_day").annotate(total_hours=Sum("hours"))
    )
    daily_hours_by_project = (
        entries.values(
            "on_day", "issue__project__business__name", "issue__project__name"
        )
        .order_by("on_day", "issue__project__business__name", "issue__project__name")
        .annotate(total_hours=Sum("hours"))
    )

    hours_per_day = {}
    for entry_hours_per_day in entries_hours_per_day:
        hours_per_day[entry_hours_per_day["on_day"]] = entry_hours_per_day[
            "total_hours"
        ]
    ...
    while running_date <= to_date:
        ...
        if not timepiece.Holiday.is_a_holiday(
            running_date
        ) and not timepiece.CalendarEvent.is_on_leave(running_date, user):
            running_days_in_week += 1

        daily_average_hours_per_week[running_date] = float(running_hours_per_week) / (
            running_days_in_week or 1
        )
...
def _create_expected_hours_series_for_graphs(request, entries, context):
    ...
    def daterange(from_date, to_date):
        return rrule(
            DAILY, dtstart=from_date, until=to_date, byweekday=(MO, TU, WE, TH, FR)
        )
    ...
    for date in daterange(from_date, to_date):
        for user in users:
            if user["usercount"] > 0:
                cumulative += 8
                entries.append({"start_time": date, "graph_value": cumulative})
...
def _get_invoices_for_graphs(request, entries, context):
    from_date, to_date = _get_filter_dates(request, context)
    invoices = bamboo_models.BambooInvoice.objects.using("bamboo").order_by(
        "dateIssued"
    )
...
def _create_cash_flow_atrate_with_expenses_series_for_graphs(request, entries, context):
    ...
    entries = list(_get_expenses_for_series(request, entries, context)) + list(
        _get_atrate_entries_for_series(request, entries, context)
    )
    entries.sort(
        key=lambda x: (
            x.start_time
            if isinstance(x, timepiece.Entry)
            else datetime.datetime(x.date.year, x.date.month, x.date.day)
        )
    )
    for entry in entries:
    ...
    return {
        "label": "Cashflow atrate (R%s)" % cumulative,
        "entries": entries,
        "yaxis": 2,
    }
...
def _create_cash_flow_atrate_series_for_graphs(request, entries, context):
    ...
    entries = list(_get_salaries_for_graphs(request, entries, context)) + list(
        _get_atrate_entries_for_series(request, entries, context)
    )
    entries.sort(
        key=lambda x: (
...
def _create_cash_flow_invoiced_series_for_graphs(request, entries, context):
    ...
    entries = list(_get_expenses_for_series(request, entries, context)) + list(
        _get_invoices_for_graphs(request, entries, context)
    )
    entries.sort(
        key=lambda x: (
            x.dateIssued if isinstance(x, bamboo_models.BambooInvoice) else x.date
        )
    )
    cumulative = 0
    for entry in entries:
        if isinstance(entry, bamboo_models.BambooInvoice):
            ...
            entry.start_time = datetime.datetime(
                entry.dateIssued.year, entry.dateIssued.month, entry.dateIssued.day
            )
        elif isinstance(entry, timepiece.Salary):
    ...
    return {
        "label": "Cashflow invoiced (R%s)" % cumulative,
        "entries": entries,
        "yaxis": 2,
    }
...
def _get_filter_dates(request, context=None):
    ...
    to_date = datetime.datetime.today().date().replace(month=1, day=1) + relativedelta(
        years=1
    )
...
def _get_filter_dates_only(request, context=None, default=None):
    ...
    if request.GET and date_form.is_valid():
        ...
        if default:
            from_date = default[0]
            to_date = default[1]
            date_form = timepiece_forms.DateOnlyForm(
...
def expense_list(request, template="timepiece/expense/index.html", context=None):
    ...
    expense_formset = timepiece_forms.expense_formset(
        request.POST or None, queryset=queryset.order_by("date")
    )
...
def invoice_list(request, template="timepiece/invoice/index.html", context=None):
    ...
    has_view_invoices = timepiece.BusinessPermissions.objects.get_or_create(
        business=context["project"].business, user=request.user
    )[0].has_edit_invoices
    ...
    invoice_formset = timepiece_forms.invoice_formset(
        request.POST or None, queryset=queryset.order_by("date_sent")
    )
    if invoice_formset.is_valid():
        ...
        return HttpResponseRedirect(
            reverse("invoice_list") + "?project_id=%s" % project_id
        )
...
def create_expense(request, context=None):
    ...
    project = timepiece.Project.objects.get(pk=project_id)
    expense = timepiece.Expense.objects.create(
        project=project,
        amount=str(amount),
...
def create_invoice(request, context=None):
    ...
    project = timepiece.Project.objects.get(pk=project_id)
    invoice = timepiece.Invoice.objects.create(
        project=project,
        date_paid=date_paid,
        date_sent=date_sent,
        description=description,
...
def time_sheet_download(request, user_id, context=None):
    ...
    to_date = datetime.datetime.strptime(
        request.GET["to_date"], "%Y%m%d"
    ).date() + datetime.timedelta(1)
    from_date = datetime.datetime.strptime(request.GET["from_date"], "%Y%m%d").date()
    entries = (
        timepiece.Entry.objects.filter_by_logged_in_user(request.user)
        .filter(start_time__gte=from_date)
        .filter(end_time__lte=to_date)
    )
    ...
    entries = (
        entries.order_by("user__username")
        .order_by("project__name")
        .order_by("start_time")
    )

    response = HttpResponse(content_type="text/csv")
    response["Content-Disposition"] = "attachment; filename=entries_%s_%s.csv" % (
    ...
    for entry in entries:
        writer.writerow(
            [
                entry.user.username.encode("utf8"),
                entry.issue.project.business.name.encode("utf8"),
                entry.issue.project.name.encode("utf8"),
                entry.start_time.strftime("%Y-%m-%d"),
                entry.hours,
                entry.comments.encode("utf8"),
            ]
        )
...
def add_project(
    request,
    business_id,
    template="timepiece/project/create_edit_project_form.html",
    context=None,
):
    context = context or {}
    business = timepiece.Business.objects.get(pk=business_id)
    ...
    has_create_sprint = timepiece.BusinessPermissions.objects.get_or_create(
        business=business, user=request.user
    )[0].has_create_sprint
    ...
    context["project"] = project
    context["business_permissions_by_user"] = timepiece.BusinessPermissions.by_user(
        business
    )
    return render(request, template, context)
...
@login_required
def add_issue(
    request, project_id, template="timepiece/project/_add_issue_form.html", context=None
):
    context = context or {}
    project = timepiece.Project.objects.filter(pk=project_id).filter_by_logged_in_user(
        request.user
    )[0]

    current_business = project.business

    context["next_issue_number"] = next_issue_number = (
        timepiece.Issue.get_next_issue_number(current_business)
    )
    context["project"] = project
    context["current_user"] = request.user
    current_user = request.user
    ...
    if bp.has_add_issue:
        new_issue_form = timepiece_forms.IssueForm(
            request.POST or None, business=current_business
        )
        plugin_form = get_interface_plugin(
            request, project.business
        ).get_create_issue_form(request.POST or None)

        if new_issue_form.is_valid() and (
            plugin_form is None or plugin_form.is_valid()
        ):
            ...
            if bp.has_estimate_own_points:
                ...
                if estimated_hours > 0:
                    timepiece.IssuePoints.objects.create(
                        user=current_user, issue=issue, points=estimated_hours
                    )

            timepiece.IssueHistory.add_history(
                request.user, issue, "created", "", issue.number
            )

            get_interface_plugin(request, project.business).create_issue(
                issue, plugin_form
            )
...
def delete_issue(request, project_id, template="", context=None):
    ...
    can_delete_issue = timepiece.BusinessPermissions.objects.get_or_create(
        business=current_business, user=current_user
    )[0].has_delete_issue
    if can_delete_issue:
        try:
            ...
            timepiece.IssueHistory.add_history(
                request.user, edited_issue, "deleted", old_id, ""
            )
            edited_issue.delete()
        except KeyError:
            edited_issue = None
...
def _augment_issue_data(
    issue, current_user, users_allowed_to_estimate_on_business, rates_by_user=None
):
    ...
    for user in users_allowed_to_estimate_on_business:
        ...
        if user_points == 0:
            ...
            completion_against_estimated_hours = (
                ((hours / user_points) * 100) if user_points > 0 else 0.0
            )
        ...
        if per_user_issue_data["completion_width"] > 100:
            ...
            per_user_issue_data["completion"] = (
                200
                if per_user_issue_data["completion"] > 200
                else per_user_issue_data["completion"]
            )
        else:
        ...
        per_user_issue_data["has_estimate"] = (
            per_user_issue_data["issue_points"] is not None
            and per_user_issue_data["issue_points"] > 0
        ) or per_user_issue_data["completion"] > 0
        ...
        if rate:
            ...
            estimated_cost = (
                user_points
                * timepiece.Rate.convert_to_full_rate(
                    issue.project, rate["billable_amount"]
                )
                * rate["velocity"]
            )
...
@login_required
def status_filter(
    request, project_id, template="timepiece/project/status_filter_popup.html"
):
    context = {}
    stati = (
        timepiece.Issue.objects.filter(project_id=project_id)
        .order_by("status2__name")
        .values_list("status2__name", flat=True)
        .distinct()
    )
    context["stati"] = stati
    context["project_id"] = project_id
...
def allowed_issue_stati(request, issue_id):
    ...
    stati = get_interface_plugin(request, issue.project.business).get_allowed_stati(
        issue
    )
    if stati is None:
        stati = [
            (x.id, x.name)
            for x in timepiece.IssueStatus.objects.filter(
                business_id=issue.project.business_id
            ).order_by("name")
        ]
...
def issue_users(request, issue_id):
    ...
    if users is None:
        users = [
            (user.id, "%s (%s)" % (user.username, user.get_full_name()))
...
def project_issues(request, pk, template="timepiece/project/issues.html", context=None):
    context = context or {}
    project = timepiece.Project.objects.filter(pk=pk).filter_by_logged_in_user(
        request.user
    )[0]
    business = project.business
    ...
    users_allowed_to_estimate_on_business = (
        business.get_users_allowed_to_estimate_on_business(request.user)
    )
    ...
    all_entries = timepiece.Entry.objects.filter(issue__project=project).order_by(
        "start_time"
    )
    cost_totals = all_entries.cost_totals_for_project(project)
    unassigned_cost_totals = timepiece.Issue.get_adhoc_timesheet_entries(
        project=project
    ).cost_totals_for_project(project)
    ...
    context["unassigned_timesheet_entries_billable"] = unassigned_cost_totals[
        "billable"
    ]
...
def get_project_detail(request, project_id, context=None):
    ...
    context["users_with_time_but_no_estimates_in_this_project"] = (
        project.get_users_with_time_but_no_estimates_in_this_project()
    )

    if len(queryset):
        issues_forms = timepiece_forms.createIssueStatusFormset(business=business)(
            request.POST or None, queryset=queryset
        )
        users_allowed_to_estimate_on_business = (
            business.get_users_allowed_to_estimate_on_business(request.user)
    ...
    unassigned_cost_totals = timepiece.Issue.get_adhoc_timesheet_entries(
        context["project"]
    ).cost_totals_for_project(context["project"])
    ...
    values = {
        ...
        "percentage_over_budget": (
            float(context["total_billable"] - project.spendable_budget)
            / project.spendable_budget
            if project.spendable_budget
            else 0
        ),
    ...
    plot_data[project_id].append(
        {"project": project.name, "values": values, "dev_hours_used": dev_hours_used}
    )
    ...
    issues_list_rendered = render(
        request, "timepiece/project/project_detail.html", context
    )
    # project_menu_rendered = render(request, "timepiece/project/_card_project_menu.html", context)
    project_menu_rendered = render(
        request, "timepiece/_navigation_project_specific_menu.html", context
    )
    project_banner_rendered = render(
        request, "timepiece/project/_project_banner.html", context
    )

    return HttpResponse(
        json.dumps(
            {
                "project": project.model_to_dict(include_business=True),
...
def open_issue(request, business_name=None, issue_number=None):
    business = (
        timepiece.Business.objects.all()
        .filter_by_logged_in_user(request.user)
        .get(name=business_name)
    )
    try:
        issue = timepiece.Issue.objects.get(
            number=issue_number, project__business=business
        )
    except timepiece.Issue.DoesNotExist:
        return HttpResponse("Unknown issue number : %s" % issue_number)
    return redirect(
        reverse(
            "highlighted_project_list",
            kwargs={"project_id": issue.project_id, "highlight_issue_id": issue.id},
        )
    )
...
def project_list(
    request,
    project_id=None,
    highlight_issue_id=None,
    business_id=None,
    template="timepiece/project/project_list.html",
    context=None,
):
    ...
    if project_id is not None:
        try:
            expanded_project = timepiece.Project.objects.filter(
                pk=project_id
            ).filter_by_logged_in_user(request.user)[0]
        except IndexError:
            raise PermissionDenied
    ...
    has_view_issues = timepiece.BusinessPermissions.objects.get_or_create(
        business=business, user=request.user
    )[0].has_view_issues
...
@login_required
def issue_detail(
    request, issue_id, template="timepiece/project/issue_detail.html", context=None
):
    context = context or {}
    issue = timepiece.Issue.objects.get(pk=issue_id)
    context["issue"] = issue
    ...
    context["is_clocked_in"] = (
        timepiece.Entry.objects.filter(user=request.user, issue_id=issue_id)
        .is_open()
        .count()
        > 0
    )

    context["timesheet_export_url"] = (
        reverse("issue_detail", args=[issue.id]) + "?timesheet_as_csv=1"
    )
    if "timesheet_as_csv" in request.GET and request.GET["timesheet_as_csv"] == "1":
        return CSVTimesheetExport(
            name="issue%d" % issue.number,
            project=project,
            timesheet_entries=issue.related_entries,
            request=request,
...
def issue_testables(
    request,
    issue_id,
    template="timepiece/project/_issue_testables_form.html",
    context=None,
):
    context = context or {}
...
@csrf_exempt
@login_required
def issue_detail_update(
    request, template="timepiece/project/issue_detail.html", context=None
):
    context = context or {}
    ...
    context["project"] = project
    context["supports_description"] = True
    context["issue_number_form"] = timepiece_forms.IssueNumberForm(
        instance=edited_issue
    )

    has_edit_description = timepiece.BusinessPermissions.objects.get_or_create(
        business=project.business, user=request.user
    )[0].has_edit_description
    ...
    try:
        ...
        timepiece.IssueHistory.add_history(
            ...
            "changed description",
            old_description,
            edited_issue.description,
        )
    except KeyError:
        pass

    get_interface_plugin(request, project.business).update_issue_description(
        edited_issue
    )
...
@csrf_exempt
@login_required
def issue_fixed_cost_update(
    request, id, template="timepiece/project/issue_detail.html", context=None
):
    context = context or {}
    ...
    try:
        ...
        timepiece.IssueHistory.add_history(
            request.user,
            edited_issue,
            "changed fixed_ctc_amount",
            old_fixed_ctc_amount,
            edited_issue.fixed_ctc_amount,
        )
    except KeyError:
        pass
...
def issue_status_update(
    ...
    project = edited_issue.project
    has_edit_status = timepiece.BusinessPermissions.objects.get_or_create(
        business=project.business, user=request.user
    )[0].has_edit_issue_states
    ...
    if not issue_status_id:
        ...
        new_status = timepiece.IssueStatus.objects.get(
            business_id=project.business_id, pk=issue_status_id
        )
    edited_issue.status2 = new_status
    timepiece.IssueHistory.add_history(
        request.user, edited_issue, "changed status", old_status, new_status
    )
    edited_issue.save()
    ...
    return HttpResponse(
        json.dumps({"new_value": str(new_status)}), content_type="application/json"
    )


@csrf_exempt
@login_required
def issue_assigned_to_update(
    request, template="timepiece/project/issue_detail.html", context=None
):
    context = context or {}
    ...
    has_assign_user = timepiece.BusinessPermissions.objects.get_or_create(
        business=project.business, user=request.user
    )[0].has_assign_user
    ...
    issue.save()
    timepiece.IssueHistory.add_history(
        request.user, issue, "assigned user", old_assigned, issue.assigned_to
    )

    get_interface_plugin(request, project.business).update_issue_assigned_to(
        issue, username
    )

    return HttpResponse(
        json.dumps(
            {"new_value": issue.assigned_to.username if issue.assigned_to else None}
        ),
...
def issue_subject_update(
    ...
    has_edit_subject = timepiece.BusinessPermissions.objects.get_or_create(
        business=project.business, user=request.user
    )[0].has_edit_subject
    ...
    try:
        ...
        timepiece.IssueHistory.add_history(
            ...
            "changed subject",
            old_subject,
            edited_issue.subject,
        )
        get_interface_plugin(request, project.business).update_issue_subject(
            edited_issue
        )
    except KeyError:
        pass
...
def issue_points_update(
    ...
    try:
        ...
        issue = timepiece.Issue.objects.get(pk=issue_id)
        edited_issue_points = timepiece.IssuePoints.objects.get_or_create(
    ...
    can_edit_points = (
        bp.has_estimate_own_points and request.user.id == edited_issue_points.user.id
    )
    ...
    if "/" in request.POST["new_value"]:
        ...
        if len(new_actual.strip()) > 0:
            timepiece.Entry.set_hours_for_user(
                user=request.user,
                issue=edited_issue_points.issue,
                new_hours=float(new_actual),
            )
            get_interface_plugin(
                request, current_project.business
            ).update_issue_actual_hours(edited_issue_points.issue)
    else:
    ...
    context["issue_number_form"] = timepiece_forms.IssueNumberForm(
        instance=edited_issue_points.issue
    )
    timepiece.IssueHistory.add_history(
        request.user,
        edited_issue_points.issue,
        "changed estimate for %s" % edited_issue_points.user,
        old_points,
        edited_issue_points.points,
    )
    get_interface_plugin(request, current_project.business).update_issue_points(
        edited_issue_points
    )
...
# @permission_required('timepiece.change_project')
@login_required
def unassigned_timesheet_entries(
    request, project_id, template="timepiece/project/issue_detail.html", context=None
):
    context = context or {}
    project = timepiece.Project.objects.filter(pk=project_id).filter_by_logged_in_user(
        request.user
    ...
    context["issue"] = {
        ...
        "sorted_related_entries": timepiece.Issue.get_adhoc_timesheet_entries(
            project=project
        ).order_by("start_time"),
    }
    context["business"] = project.business
    ...
    context["timesheet_export_url"] = (
        reverse("unassigned_timesheet_entries", args=[project.id])
        + "?timesheet_as_csv=1"
    )
    if "timesheet_as_csv" in request.GET and request.GET["timesheet_as_csv"] == "1":
        return CSVTimesheetExport(
            name="noissue",
            project=project,
            timesheet_entries=context["issue"]["related_entries"],
            request=request,
        ).render_to_response(context)
...
def all_timesheet_entries(
    ...
    entries = timepiece.Entry.objects.filter(issue__project=project).order_by(
        "start_time"
    )
    context["supports_description"] = False
    context["current_user"] = request.user
    ...
    context["timesheet_export_url"] = (
        reverse("all_timesheet_entries", args=[project.id]) + "?timesheet_as_csv=1"
    )
    if "timesheet_as_csv" in request.GET and request.GET["timesheet_as_csv"] == "1":
        return CSVTimesheetExport(
            name="all",
...
@csrf_exempt
@login_required
def view_project_rates(
    request, project_id, template="timepiece/project/view_rates.html", context=None
):
    context = context or {}
    project = timepiece.Project.objects.filter(pk=project_id).filter_by_logged_in_user(
        request.user
    )[0]
    has_view_ctc_billable_rates = timepiece.BusinessPermissions.objects.get_or_create(
        business=project.business, user=request.user
    )[0].has_view_ctc_billable_rates
    ...
    context["time_tracking_mode_options"] = ",".join(
        list([x for x, y in timepiece.Rate.TIME_TRACKING_MODES])
    )
...
@csrf_exempt
@login_required
def view_rates_summary(
    request,
    project_id,
    template="timepiece/project/view_rates_summary.html",
    context=None,
):
    context = context or {}
    project = timepiece.Project.objects.filter(pk=project_id).filter_by_logged_in_user(
...
def edit_project_rate(request, project_id):
    ...
    if field_name.startswith("project__"):
        try:
            ...
            return HttpResponse(request.POST["original_value"])
        if (
            field_name == "project__ratio_management"
            and project.ratio_management != new_value
        ):
...
@login_required
@permission_required("timepiece.change_project")
def edit_default_user_rates(
    request, template="timepiece/person/edit_default_user_rates.html", context=None
):
    if not request.user.is_superuser:
        return HttpResponse("")
    context = context or {}
    context["users"] = User.objects.all()
    formset = timepiece_forms.rate_formset(
        request.POST or None,
        queryset=timepiece.UserProfile.objects.all().order_by("user__username"),
    )
    if formset.is_valid():
        formset.save()
...
def _update_all_project_users():
    ...
    for profile in timepiece.UserProfile.objects.all():
        for project in timepiece.Project.objects.exclude(users=profile.user):
            if (
                timepiece.Entry.objects.filter(
                    user=profile.user, issue__project=project
                ).count()
                > 0
            ):
                timepiece.ProjectRelationship.objects.get_or_create(
                    user=profile.user, project=project
                )


@login_required
def income_summary(
    request, template="timepiece/graphs/income_summary.html", context=None
):
    context = context or {}
    if not request.user.is_superuser:
        return HttpResponse("")
    ...
    for entry in entries:
        ...
        per_business.setdefault(
            entry.issue.project.business.name, {"ctc": 0, "billable": 0, "hours": 0}
        )
        per_business[entry.issue.project.business.name]["ctc"] += entry.atrate
        per_business[entry.issue.project.business.name][
            "billable"
        ] += entry.atbillablerate
        per_business[entry.issue.project.business.name]["hours"] += entry.hours

        per_user_per_business.setdefault(entry.user.username, {})
        per_user_per_business[entry.user.username].setdefault(
            entry.issue.project.business.name, {"ctc": 0, "billable": 0, "hours": 0}
        )
        per_user_per_business[entry.user.username][entry.issue.project.business.name][
            "ctc"
        ] += entry.atrate
        per_user_per_business[entry.user.username][entry.issue.project.business.name][
            "billable"
    ...
    context["invoices_sent"] = timepiece.Invoice.objects.filter_by_logged_in_user(
        request.user
    ...
    context["invoices_paid"] = timepiece.Invoice.objects.filter_by_logged_in_user(
        request.user
    ).filter(Q(date_sent__gte=from_date) & Q(date_sent__lte=to_date))
...
def issue_search(
    ...
    template="timepiece/project/issue_search_results.html",
    context=None,
):
    context = context or {}

    active_project = (
        timepiece.Project.objects.get(pk=active_project_id)
        if active_project_id
        else None
    )
    active_business = (
        timepiece.Business.objects.get(pk=active_business_id)
    ...
    if active_project is not None:
        active_issues = timepiece.Issue.objects.filter(
            project__id=active_project.id
        ).filter(Q(number__icontains=search_term) | Q(subject__icontains=search_term))
        active_issues = active_issues.order_by(
            "project__business__name", "project__name", "subject"
        )
        active_issues = [
            x for x in active_issues if x.project.can_view_by_user(request.user)
        ]
        active_business = None

    elif active_business is not None:
        active_issues = timepiece.Issue.objects.filter(
            project__business__id=active_business.id
        ).filter(Q(number__icontains=search_term) | Q(subject__icontains=search_term))
        active_issues = active_issues.order_by(
            "project__business__name", "project__name", "subject"
        )
        ...
        active_sprints = timepiece.Project.objects.filter(
            business__id=active_business.id
        ).filter(
    ...
    issues = timepiece.Issue.objects.filter(
        Q(number__icontains=search_term) | Q(subject__icontains=search_term)
    )
    ...
    sprints = timepiece.Project.objects.filter(
        Q(name__icontains=search_term)
        | Q(id__icontains=search_term)
        | Q(description__icontains=search_term)
    ...
    if response_mode == "im_feeling_lucky":
        ...
        if len(issues) > 0:
            issue = issues[0]
            res["best_match"] = {
                "javascript": "imp.nav.show_issue("
                + str(issue.id)
                + ", "
                + str(issue.project.id)
                + ", '"
                + reverse("highlighted_project_list", args=[issue.project.id, issue.id])
                + "' );"
            }
        elif len(sprints) > 0:
            sprint = sprints[0]
            res["best_match"] = {
                "javascript": "imp.nav.show_sprint("
                + str(sprint.id)
                + ", '"
                + reverse("project_list", args=[sprint.id])
                + "' );"
            }
        elif len(businesses) > 0:
            business = businesses[0]
            res["best_match"] = {
                "javascript": "imp.nav.show_business("
                + str(business.id)
                + ", '"
                + reverse("closed_project_list", args=[business.id])
                + "');"
            }
...
def show_timeline(request, project_id):
    ...
    entries_qs = timepiece.Entry.objects.filter(
        issue__project__business=project.business
    ).order_by("start_time")
...
def show_permissions(request, business_id):
    ...
    permission_forms = timepiece_forms.permissions_formset(
        request.POST or None, queryset=permissions_set
    )

    if permission_forms.is_valid():
        has_edit_permissions = timepiece.BusinessPermissions.objects.get_or_create(
            business=business, user=request.user
        )[0].has_edit_permissions
...
def get_project_row(request, project_id, context=None):
    ...
    context["project"] = project
    r = render(request, "timepiece/project/_project_list_item.html", context)
    return r
...
def get_issue_row(request, issue_id):
    ...
    r = render(request, "timepiece/project/_issue_entry_row.html", context)
...
def add_issue_comment(request, issue_id):
    ...
    timepiece.IssueHistory.add_history(
        request.user,
        issue,
        "added comment %s" % new_comment.id,
        "",
        new_comment.comment,
    )
...
def delete_issue_comment(request, comment_id):
    ...
    timepiece.IssueHistory.add_history(
        request.user, issue, "deleted comment %s" % old_comment_id, old_comment_text, ""
    )
    return HttpResponse("ok")
...
def add_issue_testable(request, issue_id):
    ...
    new_testable = Testable.objects.create(steps=text, issue=issue, order=max_order + 1)

    timepiece.IssueHistory.add_history(
        request.user,
        issue,
        "added testable %s" % new_testable.id,
        "",
        new_testable.steps,
    )
...
def delete_issue_testable(request, testable_id):
    ...
    timepiece.IssueHistory.add_history(
        request.user,
        issue,
        "deleted testable %s" % old_testable_id,
        old_testable_text,
        "",
    )
    return HttpResponse("ok")
...
def add_issue_attachment(request, issue_id):
    ...
    timepiece.IssueHistory.add_history(
        request.user, issue, "added attachment", "", f.name
    )
    return HttpResponse("ok")
...
def delete_issue_attachment(request, attachment_id):
    ...
    attachment.delete()
    timepiece.IssueHistory.add_history(
        request.user, issue, "deleted attachment", attachment.name, ""
    )
    return HttpResponse("ok")
...
def sortable_issue_update(request, project_id, context=None):
    ...
    timepiece.Issue.objects.filter(pk__in=ordered_issue_ids).update(
        project_id=new_project_id
    )
    timepiece.ProjectIssueOrder.order_like_this(
        project_id=new_project_id, ordered_issue_ids=ordered_issue_ids
    )
    return HttpResponse("")
...
def sortable_project_update(request):
    ...
    timepiece.BusinessProjectOrder.order_like_this(
        business_id=first_project.business_id, ordered_project_ids=ordered_project_ids
    )
    return HttpResponse("")
...
def sprint_report_settings(request, project_id, context=None):
    ...
    if "only_these_issues" in request.GET and request.GET["only_these_issues"]:
        only_these_issues = [
            timepiece.Issue.objects.get(pk=issue_id)
            for issue_id in request.GET["only_these_issues"].split(",")
            if issue_id
        ]
    else:
        only_these_issues = None
    context["quote_form"] = timepiece_forms.SprintQuoteReportSettingsForm(
        project, bp, only_these_issues
    )
    context["invoice_form"] = timepiece_forms.SprintInvoiceReportSettingsForm(
        project, bp, only_these_issues
    )
    context["project"] = project
    return render(request, "timepiece/project/sprint_report_settings.html", context)
...
def sprint_report(request, project_id, context=None):
    ...
    try:
        ...
        if "authenticate_token" in DATA and "authenticate_username" in DATA:
            ...
            try:
                user = timepiece.UserProfile.objects.get(
                    authenticate_token=authenticate_token, user__username=username
                ).user
        ...
        if (
            ...
            and DATA["output_format"] == "pdf"
            and "HTTP_REFERER" in request.META
        ):
            url = request.build_absolute_uri(
                reverse("sprint_report", kwargs={"project_id": project.id})
            )
            ...
            document = timepiece.BusinessDocument.objects.create(
                project=project,
                business=project.business,
                filename=filename,
                doc_type=doc_type,
        ...
        quote_form = timepiece_forms.SprintQuoteReportSettingsForm(
            project, bp, issues, DATA
        )
        invoice_form = timepiece_forms.SprintInvoiceReportSettingsForm(
            project, bp, issues, DATA
        )
        ...
        if DATA["report_type"] == "Quote" and quote_form.is_valid():
            ...
            DATA["report_type"] == "Invoice" or DATA["report_type"] == "Summary"
        ) and invoice_form.is_valid():
            form = invoice_form
            template = "timepiece/project/sprint_invoice_report.html"
        else:
            raise Exception(
                "Unknown report type or invalid params: %s %s %s"
                % (
                    DATA["report_type"],
                    str(invoice_form.errors),
                    str(quote_form.errors),
                )
            )
        ...
        if form.is_valid():
            ...
            if "only_these_issue_numbers" in form.cleaned_data:
                issues = issues.filter(
                    number__in=form.cleaned_data["only_these_issue_numbers"]
                )
        ...
        if form == quote_form:
            context["estimate_stats"] = project.estimate_stats(
                issues,
                preferred_user_id=form.cleaned_data["preferred_user_for_estimates"],
            )
        context["issues"] = issues
        context["form"] = form
        context["project"] = project
        ...
        if "start" in context["settings"]:
            stats = project.cache_stats(
                start=context["settings"]["start"],
                end=context["settings"]["end"],
        ...
        most_recent_quote_document = (
            timepiece.BusinessDocument.objects.filter(project=project)
            .order_by("-id")
            .first()
        )
        new_quote_default_args = {
            "status": "sent to client",
            "project": project.id,
            "internal_comment": "Created by %s" % request.user,
            "amount": int(
                project.new_stats["total"]["hours_billable_with_scope_creep"] or 0
            ),
            "quote_document": (
                most_recent_quote_document.id if most_recent_quote_document else None
            ),
            "currency_symbol": "R",
        }
        context["url_capture_quote"] = (
            reverse("invoicing:new_quote")
            + "?"
            + urllib.urlencode(new_quote_default_args)
        )
...
def edit_issue_number(request, issue_id, context=None):
    ...
    if form.is_valid():
        form.save()
        timepiece.IssueHistory.add_history(
            request.user, issue, "issue number", old_number, issue.number
        )
...
def show_issue_history(
    request, issue_id, template="timepiece/project/issue_history.html", context=None
):
    context = context or {}
    issue = timepiece.Issue.objects.get(pk=issue_id)
    project = issue.project
...
@login_required
def view_business_documents(
    request,
    business_id,
    template="timepiece/project/business_documents.html",
    context=None,
):
    context = context or {}
    business = timepiece.Business.objects.get(pk=business_id)
    ...
    if not bp.has_view_documents:
        return HttpResponse(
            "Sorry, you don't have permission to view business documents"
        )

    form = timepiece_forms.NewBusinessDocumentForm(
    ...
    if form.is_valid():
        ...
        return HttpResponseRedirect(
            reverse("view_business_documents", args=[business_id])
        )

    context["new_doc_form"] = form
    context["generate_doc_form"] = timepiece_forms.GenerateBusinessDocumentForm(
        request.POST or None
    )
    context["business"] = business
    context["documents"] = (
        business.documents.all().filter(deleted=False).order_by("-created_at")
    )
    return render(request, template, context)
...
def download_business_document(
    ...
    if not bp.has_view_documents:
        return HttpResponse(
            "Sorry, you don't have permission to view this business document"
        )

    context["business"] = business
    context["documents"] = business.documents.all().order_by("-created_at")

    response = download_media(
        request, document.doc.name, content_type=document.mime_type
    )
...
def edit_business_document(
    request,
    document_token,
    template="timepiece/project/edit_business_document.html",
    context=None,
):
    context = context or {}
    document = timepiece.BusinessDocument.objects.get(token=document_token)
    ...
    form = timepiece_forms.EditBusinessDocumentForm(
    ...
    if form.is_valid():
        ...
        return HttpResponseRedirect(
            reverse("view_business_documents", args=[business.id])
        )
...
@csrf_exempt
def generate_preview_business_document(
    request,
    business_id,
    template="timepiece/project/generate_business_document_preview.html",
    context=None,
):
    context = context or {}
    business = timepiece.Business.objects.get(pk=business_id)
    ...
    if not bp.has_view_documents:
        return HttpResponse(
            "Sorry, you don't have permission to create business documents"
        )
    ...
    form = timepiece_forms.GenerateBusinessDocumentForm(
    ...
    if form.is_valid():
        ...
        if (
            "render_mode" in request.POST
            and request.POST["render_mode"] == "generate"
            and "HTTP_REFERER" in request.META
        ):
...
@login_required
def issue_checkbox_context_menu(
    request,
    project_id,
    template="timepiece/project/issue_checkbox_context_menu.html",
    context=None,
):
    context = context or {}
    ...
    checked_issue_ids = [
        x for x in raw_checked_issue_numbers.split(",") if len(x.strip()) > 0
    ]
    ...
    other_projects = [
        p
        for p in timepiece.Project.objects.filter(
            business=from_project.business
        ).exclude(pk=from_project.id)
        if p.is_open
    ]
    ...
    context["state_select_form"] = (
        timepiece_forms.IssueCheckboxContextMenuSelectByStateForm(from_project)
    )
    context["state_change_form"] = (
        timepiece_forms.IssueCheckboxContextMenuChangeStateForm(from_project)
    )
    context["assignee_change_form"] = (
        timepiece_forms.IssueCheckboxContextMenuChangeAssigneeForm(from_project)
    )
    context["move_above_issue_form"] = (
        timepiece_forms.IssueCheckboxContextMenuActiveIssueForm(
            from_project, "Move above"
        )
    )
    context["move_below_issue_form"] = (
        timepiece_forms.IssueCheckboxContextMenuActiveIssueForm(
            from_project, "Move below"
        )
    )
    context["bulk_change_issue_adhoc_form"] = (
        timepiece_forms.IssueCheckboxContextMenuChangeIssueAdhocForm(
            from_project, "Change adhoc"
        )
    )
...
def move_issue_to_project(request):
    ...
    timepiece.IssueHistory.add_history(
        ...
        "moved project",
        unicode(old_project),
        unicode(dest_project),
    )
    get_interface_plugin(request, dest_project.business).move_issue(
        issue, old_project=old_project
    )
...
def bulk_move_issues_to_project(request, dest_project_id, context=None):
    ...
    messages.info(
        request, "%d issues moved to %s" % (len(selected_issue_ids), dest_project)
    )
...
def bulk_change_issue_state(request, context=None):
    selected_issue_ids = request.session["selected_issue_ids_for_context_menu"]
    selected_project = timepiece.Project.objects.get(
        pk=request.session["selected_issue_project_id"]
    )

    form = timepiece_forms.IssueCheckboxContextMenuChangeStateForm(
        selected_project, request.GET or None
    )
    ...
    if not form.cleaned_data["status"]:
        ...
        new_status = timepiece.IssueStatus.objects.get_or_create(
            pk=form.cleaned_data["status"], business=selected_project.business
        )[0]
    for selected_issue_id in selected_issue_ids:
        issue = timepiece.Issue.objects.get(pk=selected_issue_id)
        if not issue.status2 or new_status != issue.status2.name:
            ...
            issue.save()
            timepiece.IssueHistory.add_history(
                request.user, issue, "changed status", old_status, new_status
            )
            get_interface_plugin(
                request, selected_project.business
            ).update_issue_status(issue)

    messages.info(
        request, "%d issues changed state to %s" % (len(selected_issue_ids), new_status)
    )
...
def bulk_change_issue_assignee(request, context=None):
    ...
    form = timepiece_forms.IssueCheckboxContextMenuChangeAssigneeForm(
        selected_project, request.GET or None
    )
    if not form.is_valid():
    ...
    for selected_issue_id in selected_issue_ids:
        ...
        if new_assignee != issue.assigned_to:
            ...
            issue.save()
            timepiece.IssueHistory.add_history(
                request.user, issue, "changed assigned user", old_assignee, new_assignee
            )
    messages.info(
        request,
        "%d issues changed assigned user to %s"
        % (len(selected_issue_ids), new_assignee),
    )
    return HttpResponseRedirect(reverse("project_list", args=[selected_project.id]))
...
def bulk_move_issue_above_issue(request, context=None):
    ...
    form = timepiece_forms.IssueCheckboxContextMenuActiveIssueForm(
        selected_project, "", request.GET or None
    )
    if not form.is_valid():
        return HttpResponse("No issue chosen: %s" % form.errors)
    focus_issue = timepiece.Issue.objects.filter(project=selected_project).get(
        pk=form.cleaned_data["focus_issue"]
    )
    ...
    for index, issue in enumerate(selected_issues):
        num_moved += 1
        timepiece.ProjectIssueOrder.insert_before(
            issue=issue, set_before_this_issue=running_issue
        )
        running_issue = issue

    messages.info(
        request,
        "%d issues moved above %s %s"
        % (num_moved, focus_issue.number, focus_issue.subject),
    )
    return HttpResponseRedirect(reverse("project_list", args=[selected_project.id]))
...
def bulk_move_issue_below_issue(request, context=None):
    ...
    for index, issue in enumerate(selected_issues):
        num_moved += 1
        timepiece.ProjectIssueOrder.insert_after(
            issue=issue, set_after_this_issue=running_issue
        )
        running_issue = issue

    messages.info(
        request,
        "%d issues moved below %s %s"
        % (num_moved, focus_issue.number, focus_issue.subject),
    )
    return HttpResponseRedirect(reverse("project_list", args=[selected_project.id]))
...
def bulk_select_issues_for_project(request, project_id, context=None):
    ...
    request.session["selected_issue_ids_for_context_menu"] = [
        x.id for x in selected_issues
    ]
...
def bulk_select_by_issue_state(request, project_id, context=None):
    ...
    form = timepiece_forms.IssueCheckboxContextMenuSelectByStateForm(
        selected_project, request.GET or None
    )

    if form.is_valid():
        state_id = form.cleaned_data["status"]
        state_to_select = timepiece.IssueStatus.objects.get(
            pk=state_id, business_id=selected_project.business_id
        )
        ...
        messages.info(
            request,
            "%d issues selected for state %s"
            % (selected_issues.count(), state_to_select.name),
        )
    else:
...
def bulk_change_issue_adhoc(request, context=None):
    ...
    form = timepiece_forms.IssueCheckboxContextMenuChangeIssueAdhocForm(
        selected_project, "Adhoc", request.GET or None
    )
    if not form.is_valid():
    ...
    for selected_issue_id in selected_issue_ids:
        ...
        if new_adhoc != issue.issue_type:
            ...
            issue.save()
            timepiece.IssueHistory.add_history(
                request.user, issue, "changed adhoc", old_adhoc, new_adhoc
            )
    messages.info(
        request,
        "%d issues changed adhoc: : %s"
        % (len(selected_issue_ids), form.cleaned_data["adhoc"]),
    )
    return HttpResponseRedirect(reverse("project_list", args=[selected_project.id]))
...
@login_required
def auto_issue_sort(
    request, project_id, template="timepiece/project/auto_issue_sort.html", context=None
):
    context = context or {}
    project = timepiece.Project.objects.filter(pk=project_id).filter_by_logged_in_user(
        request.user
    ...
    if "ordered_states" in request.POST:
        ...
        return HttpResponse(
            json.dumps({"redirect_url": reverse("project_list", args=[project.id])})
        )
    else:
        context["states"] = [
            x["status2__name"]
            for x in project.issues.order_by("status2__name")
...
def calendar(request, template="timepiece/calendar/calendar.html", context=None):
    ...
    context["form_new_event"] = timepiece_forms.CalendarEventCreateForm(
        context["users"], context["businesses"], request.POST or None
    )
    context["update_form"] = timepiece_forms.CalendarEventUpdateForm(
        context["users"], context["businesses"]
    )
    return render(request, template, context)
...
def calendar_events(request, context=None):
    ...
    events = (
        [_create_js_calendar_event(event) for event in context["calendar_events"]]
    ...
    if type(entry) == dict:
        ...
        try:
            if "business__business_id" in entry:
                business = timepiece.Business.objects.get(
                    pk=entry["business__business_id"]
                )
            else:
                business = None
        except timepiece.Business.DoesNotExist:
        ...
        return {
            "id": None,
            "title": "%s %s (%.2f hours)"
            % (user.username, business.name if business else "all businesses", hours),
            "allDay": False,
            "start": start_time.strftime("%Y-%m-%d %H:%M"),
            "end": end_time.strftime("%Y-%m-%d %H:%M"),
            "business_id": None,
            "user_id": user.id,
            "description": "%s (%.2f hours)"
            % (business.name if business else "none", hours),
            "event_type": "actual",
        ...
        return {
            "id": entry.id,
            "title": "A:%s (%s, %s hrs)"
            % (entry.issue.project.business.name, entry.user.username, entry.hours),
            "allDay": False,
...
def _create_js_calendar_event(event):
    ...
    if event.is_open and event.start < datetime.datetime.today() - relativedelta(
        days=1
    ):
        res["is_overdue"] = True
...
def _populate_calendar_events(request, context):
    try:
        bps_for_scheduling = timepiece.BusinessPermissions.objects.filter(
            can_be_scheduled=True
        )
        users = timepiece.User.objects.filter(
            pk__in=[
                x["user"]
                for x in bps_for_scheduling.order_by("user__username").values("user")
            ]
        )

        bps_logged_in_user_can_view = timepiece.BusinessPermissions.objects.filter(
            can_view_calendar=True, user=request.user
        )
        bp_businesses_for_scheduling = [
            x["business"]
            for x in bps_for_scheduling.order_by("business__name").values("business")
        ]
        businesses = timepiece.Business.objects.filter(
            pk__in=bp_businesses_for_scheduling
        )

        if not request.user.is_superuser:
            bp_businesses_logged_in_user_can_view = [
                x["business"]
                for x in bps_logged_in_user_can_view.order_by("business__name").values(
                    "business"
                )
            ]
        ...
        projects = (
            timepiece.Project.objects.filter(business__in=businesses)
            .filter_in_dev()
        ...
        calendar_events = (
            timepiece.CalendarEvent.objects.filter(user__in=users)
            .filter(Q(business__in=businesses) | Q(business__isnull=True))
            .distinct()
            .order_by("start")
        )
        ...
        filter_form = timepiece_forms.CalendarFilterForm(
            users, businesses, request.GET or None, initial={"users": [request.user]}
        )
        if filter_form.is_valid():
            calendar_events, entry_events, holiday_events = filter_form.save(
                calendar_events, entry_events, holiday_events
            )
...
def create_calendar_event(request, context=None):
    ...
    form_new_event = timepiece_forms.CalendarEventCreateForm(
        context["users"], context["businesses"], request.POST or None
    )
    if form_new_event.is_valid():
        ...
        if form_new_event.cleaned_data["business"] is not None:
            ...
            bp = timepiece.BusinessPermissions.for_user(
                request.user, form_new_event.cleaned_data["business"]
            )
            ...
            if (
                ...
                and not request.user == form_new_event.cleaned_data["user"]
            ):
...
def update_calendar_event(request, event_id, context=None):
    ...
    form = timepiece_forms.CalendarEventUpdateForm(
        context["users"],
        context["businesses"],
        request.POST or None,
    ...
    if form.is_valid():
        ...
        if calendar_event.business is not None:
            bp = timepiece.BusinessPermissions.for_user(
                request.user, calendar_event.business
            )
            if not bp.has_edit_calendar:
                raise PermissionDenied
        else:
            if not request.user.is_superuser and (
                not request.user == calendar_event.user
                or not request.user == form.cleaned_data["user"]
            ):
                # Can't create events for other people unless you're the admin
                raise PermissionDenied
    ...
    logger.exception(
        "Couldn't update calendar event %s because %s"
        % (calendar_event.id, form.errors)
    )
...
@login_required
@csrf_exempt
def render_calendar_scheduled_sprints(
    request, template="timepiece/calendar/_scheduled_sprints.html", context=None
):
    context = context or {}
    _populate_calendar_events(request, context)
    ...
    if request.user.is_superuser:
        context["projects_with_unscheduled_hours"] = [
            p
            for p in context["projects"].filter_in_dev()
            if p.min_unscheduled_hours() > 0 and p.min_estimate_hours() > 0
        ]
        context["projects_with_fully_scheduled_hours"] = [
            p
            for p in context["projects"].filter_in_dev()
            if p.min_unscheduled_hours() < 1 and p.min_estimate_hours() > 0
        ]
    else:
        context["projects_with_unscheduled_hours"] = []
        context["projects_with_fully_scheduled_hours"] = []
...
@login_required
def business_cost_summary(
    request,
    business_id,
    template="timepiece/project/business_cost_summary.html",
    context=None,
):
    context = context or {}
    business = timepiece.Business.objects.get(pk=business_id)
    ...
    for project in (
        ...
        for mode in timepiece.TIME_TRACKING_MODES:
            ...
            running_costs_per_role[mode][
                "hours_billable_core_rate"
            ] += project.new_stats["per_role"][mode]["hours_billable_core_rate"]
    ...
    context["sundry_totals"] = {
        "invoiced": sundry_invoiced,
        "paid": sundry_paid,
        "owed": sundry_owed,
    }

    context["project_totals"] = {
...
def project_status_update(request, project_id):
    ...
    has_edit_status = timepiece.BusinessPermissions.objects.get_or_create(
        business=project.business, user=request.user
    )[0].has_edit_project_states
    if not has_edit_status:
        raise PermissionDenied
    project.status3 = timepiece.ProjectStatus.objects.get_or_create(
        business=project.business, name=request.POST["selected_value"]
    )[0]
    project.save()
    return HttpResponse(
        json.dumps({"new_value": project.status3.name, "is_open": project.is_open}),
        content_type="application/json",
    )
...
def allowed_project_stati(request, project_id):
    return HttpResponse(
        json.dumps(timepiece.Project.PROJECT_STATUSES), content_type="application/json"
    )
...
def business_comments(request, business_id):
    ...
    if bp.has_edit_business_comments:
        comment = timepiece.BusinessComment.objects.get_or_create(
            business=business, defaults={"modified_by": request.user}
        )[0]
        form = timepiece_forms.BusinessCommentForm(
        ...
        if form.is_valid():
            ...
            comment.save()
            form.save_m2m()
            timepiece.BusinessHistory.add_history(
                request.user, business, "comment", old_comment, comment.comment
            )
    ...
    return render(request, "timepiece/project/project_comments.html", context)


@login_required
def show_business_history(
    request,
    business_id,
    template="timepiece/project/business_history.html",
    context=None,
):
    context = context or {}
    business = timepiece.Business.objects.get(pk=business_id)
    ...
    if not bp.has_edit_project_detail:
        return HttpResponse(
            "Sorry, you don't have permission to view the business history"
        )
...
@login_required
def render_checklist_navigation(
    request,
    business_id,
    template="timepiece/project/_checklist_menu.html",
    context=None,
):
    context = context or {}
    if not business_id:
...
@login_required
def render_checklist_status_icon_traffic(
    request,
    business_id,
    template="timepiece/project/_checklist_status_icon_traffic.html",
    context=None,
):
    context = context or {}
    business = timepiece.Business.objects.get(pk=business_id)
...
@login_required
def render_checklist_status_icon_dev(
    request,
    business_id,
    template="timepiece/project/_checklist_status_icon_dev.html",
    context=None,
):
    context = context or {}
    business = timepiece.Business.objects.get(pk=business_id)
...
@login_required
def render_checklist_status_icon_finance(
    request,
    business_id,
    template="timepiece/project/_checklist_status_icon_finance.html",
    context=None,
):
    context = context or {}
    business = timepiece.Business.objects.get(pk=business_id)
...
@login_required
def traffic_checklist(
    request,
    business_id,
    template="timepiece/project/traffic_checklist.html",
    context=None,
):
    context = context or {}
    business = timepiece.Business.objects.get(pk=business_id)
    bp = timepiece.BusinessPermissions.for_user(request.user, business)
    if not (
        bp.has_do_traffic_checklist
        and bp.has_view_ctc_billable_rates
        and bp.has_view_ctc_rates
    ):
    ...
    form = timepiece_forms.TrafficChecklistForm(
        request.POST or None, instance=checklist
    )
    if form.is_valid():
    ...
    context["business"] = business
    context["previous_checklists"] = (
        timepiece.TrafficChecklist.objects.all()
        .filter(business=business)
        .order_by("-pk")[1:5]
    )
    return render(request, template, context)
...
@login_required
def dev_checklist(
    request, business_id, template="timepiece/project/dev_checklist.html", context=None
):
    context = context or {}
    business = timepiece.Business.objects.get(pk=business_id)
    bp = timepiece.BusinessPermissions.for_user(request.user, business)
    if not (
        bp.has_do_dev_checklist
        and bp.has_view_ctc_billable_rates
        and bp.has_view_ctc_rates
    ):
    ...
    context["business"] = business
    context["previous_checklists"] = (
        timepiece.DevChecklist.objects.all()
        .filter(business=business)
        .order_by("-pk")[1:5]
    )
    return render(request, template, context)
...
@login_required
def finance_checklist(
    request,
    business_id,
    template="timepiece/project/finance_checklist.html",
    context=None,
):
    context = context or {}
    business = timepiece.Business.objects.get(pk=business_id)
    ...
    context["business"] = business
    context["previous_checklists"] = (
        timepiece.FinanceChecklist.objects.all()
        .filter(business=business)
        .order_by("-pk")[1:5]
    )
...
def recalculate_all_checklists(request, context=None):
    ...
    for business in timepiece.Business.objects.filter_has_at_least_one_open_project():
        timepiece.DevChecklist.get_todays_checklist(
            request.user, business
        ).recalculate_all()
        timepiece.TrafficChecklist.get_todays_checklist(
            request.user, business
        ).recalculate_all()
        timepiece.FinanceChecklist.get_todays_checklist(
            request.user, business
        ).recalculate_all()
...
def user_notifications(
    request, template="timepiece/project/user_notifications.html", context=None
):
    context = context or {}
    timepiece.UserNotification.create_default_notifications(request.user)
    notifications = timepiece.UserNotification.objects.filter(
        user=request.user, seen=False, applies_on=datetime.datetime.today().date()
    )
...
def seen_user_notification(request, notification_id, context=None):
    timepiece.UserNotification.objects.filter(
...
def dashboard(request, template="timepiece/dashboard/dashboard.html"):
    ...
    for x in range(num_months):
        ...
        context["quotes_by_month"].append(
            {
                "month": date_from,
                "quotes_by_sent_to_client_at": Quote.objects.filter(
                    sent_to_client_at__gte=date_from, sent_to_client_at__lt=running_now
                ),
                "quotes_by_accepted_at": Quote.objects.filter(
                    accepted_at__gte=date_from, accepted_at__lt=running_now
                ),
            }
        )

        context["invoices_by_month"].append(
            {
                "month": date_from,
                "invoices_waiting": Invoice.objects.filter_by_logged_in_user(
                    request.user
                ).filter(
                    status="open",
                    payment_due__gte=date_from,
                    payment_due__lt=running_now,
                ),
                "invoices_paid": Invoice.objects.filter_by_logged_in_user(
                    request.user
                ).filter(
                    status="paid",
                    payment_due__gte=date_from,
        ...
        entries = timepiece.Entry.objects_for_reporting.filter(
            start_time__gte=date_from, start_time__lte=running_now
        )
        salaries = timepiece.Salary.objects.filter(
            date__gte=date_from, date__lt=running_now, amount__gt=0
        )
        business_days = timepiece.Holiday.business_days_in_month(date_from)
        context["employees_by_month"].append(
            {
                "month": date_from,
                "salaries": salaries,
                "entries": entries,
    ...
    def create_checklist_info(
        state, checklist_name, business, checklist, checklist_errors=checklist_errors
    ):
        # checklist.recalculate_all()
        if checklist.is_ok():
            return
        checklist_errors.append(
            (
                business,
                "(%s) %d %s errors"
                % (state, checklist.items.filter(passed=False).count(), checklist_name),
    ...
    def create_checklist_summary(state, businesses):
        for business in businesses:
            create_checklist_info(
                state,
                "dev",
                business,
                timepiece.DevChecklist.get_todays_checklist(request.user, business),
            )
            create_checklist_info(
                state,
                "traffic",
                business,
                timepiece.TrafficChecklist.get_todays_checklist(request.user, business),
            )
            create_checklist_info(
                state,
                "finance",
                business,
                timepiece.FinanceChecklist.get_todays_checklist(request.user, business),
            )
...
@csrf_exempt
def quick_clocker(
    request, template="timepiece/time-sheet/quick_clocker.html", context=None
):
    context = context or {}

    users = timepiece.BusinessPermissions.get_users_who_can_capture_time().order_by(
        "username"
    )
    users = users.filter(pk=request.user.id)
    allowed_projects = timepiece.Project.objects.all().filter_by_logged_in_user(
        request.user
    )
    ...
    clock_in_form = timepiece_forms.QuickClockerForm(
        request.user, users, projects, request.POST or None
    )
    if clock_in_form.is_valid():
        ...
        new_entry = timepiece.Entry.objects.create(
            user=clock_in_form.cleaned_data["user"],
        ...
        for open_entry in (
            timepiece.Entry.objects.filter(user=new_entry.user)
            .exclude(pk=new_entry.id)
            .is_open()
        ):
        ...
        messages.info(
            request,
            "%s clocked into %s" % (new_entry.user.username, project.long_name()),
        )

        return HttpResponseRedirect(reverse("quick_clocker"))

    allowed_entries = timepiece.Entry.objects.all().filter_by_logged_in_user(
        request.user
    )
    ...
    context["clocked_in_entries"] = users_quick_clocker_entries.is_open().order_by(
        "user__username"
    )
    context["recently_clocked_out_entries"] = (
        users_quick_clocker_entries.is_closed().order_by("-date_updated")[0:15]
    )
    context["recent_dev_entries"] = (
        users_quick_clocker_entries.filter(source="emacs")
        .is_closed()
        .order_by("-end_time")[0:20]
    )
    logged_in_users_active_entry = users_quick_clocker_entries.filter(
        user=request.user
    ).first()
    if context.get("clock_out_form", None) is None:
        context["clock_out_form"] = timepiece_forms.QuickClockerClockOutForm(
            context["clocked_in_entries"],
            initial={
                "entry": logged_in_users_active_entry,
...
def quick_clocker_clock_out(request):
    ...
    entries = (
        timepiece.Entry.objects.all()
        .filter(user=request.user, source="quick_clocker")
        .is_open()
    )
    clock_out_form = timepiece_forms.QuickClockerClockOutForm(
        entries, request.POST or None, initial={"clock_out_time": api.localised_today()}
    )
    if clock_out_form.is_valid():
        ...
        messages.info(
            request,
            "%s clocked out of %s"
            % (entry.user.username, entry.issue.project.long_name()),
        )
        return HttpResponseRedirect(reverse("quick_clocker"))
...
def quick_clocker_edit_entry(request, entry_id=None):
    entry = (
        timepiece.Entry.objects_original.all()
        .filter(user=request.user)
        .get(pk=entry_id)
    )
    projects = (
        timepiece.Project.objects.filter(business=entry.issue.project.business)
        .filter_can_add_dev_time_states()
        .order_by("name")
    )
    form = timepiece_forms.QuickClockerEditEntry(
        projects, request.POST or None, instance=entry
    )
    if form.is_valid():
        form.save()
        if not entry.issue:
        ...
        return HttpResponseRedirect(
            reverse("quick_clocker_edit_entry", kwargs={"entry_id": entry_id})
        )
...
def _get_quick_clocker_issue(project, user):
    ...
    try:
        issue = timepiece.Issue.objects.get(
            project=project, subject=issue_subject, assigned_to=user
        )
    except timepiece.Issue.MultipleObjectsReturned:
        issue = timepiece.Issue.objects.filter(
            project=project, subject=issue_subject, assigned_to=user
        ).last()
    except timepiece.Issue.DoesNotExist:
        issue = timepiece.Issue.objects.create(
            project=project,
            subject=issue_subject,
            auto_created_during_import=True,
            issue_type="issue",
            status2=timepiece.IssueStatus.objects.get_or_create(
                name="quick_clocker", business=project.business
            )[0],
            assigned_to=user,
            created_by=user,
...
def scheduler(request, template="timepiece/scheduler/scheduler.html", context=None):
    ...
    schedule_filter_form = timepiece_forms.ScheduleFilterForm(
        request.GET or None,
        initial={
    ...
    if schedule_filter_form.is_valid():
        date_from = datetime.datetime(
            year=schedule_filter_form.cleaned_data["year"],
    ...
    businesses = (
        timepiece.Business.objects.filter_has_can_add_dev_time_projects().distinct()
    )
    users = timepiece.BusinessPermissions.get_users_who_can_capture_time().order_by(
        "username"
    )

    schedules = timepiece.Schedule.objects.filter(
        scheduled_date__gte=date_from, scheduled_date__lt=date_to
    )

    actuals = timepiece.Entry.objects.filter(
        start_time__gte=date_from, start_time__lt=date_to
    )
    ...
    context["public_holidays"] = timepiece.Holiday.objects.filter(
        applies_on__gte=date_from, applies_on__lt=date_to
    )
...
def schedule_edit(request, business_id, user_id, scheduled_date):
    ...
    schedule = timepiece.Schedule.objects.get_or_create(
        business_id=business_id,
        user_id=user_id,
        scheduled_date=scheduled_date,
        defaults={"num_hours": 0},
    )[0]
    ...
    if form.is_valid():
        ...
        return HttpResponse(
            json.dumps(
                {
                    "hours_captured": schedule.num_hours,
                    "hours_for_user": "%d" % (schedules.hours_for_user(user_id) or 0),
                    "hours_for_user_is_over": (actuals.hours_for_user(user_id) or 0)
                    > (schedules.hours_for_user(user_id) or 0),
                    "hours_for_business": "%d"
                    % (schedules.hours_for_business(business_id) or 0),
                    "hours_total": schedules.hours(),
                    "scheduled_status_msg": scheduled_status(
                        context={"date": date_from, "schedules": schedules},
                        user=User.objects.get(pk=user_id),
                    ),
                    "billable_for_user": _format_money(
                        schedules.billable_for_user(user_id)
                    ),
                    "billable_for_business": _format_money(
                        schedules.billable_for_business(business_id)
                    ),
...
def send_calendar_invite(request, event_id):
    ...
    try:
        ...
        logger.exception(ex)
        return HttpResponse(json.dumps({"status": "failed", "error_msg": str(ex)}))


@login_required
@csrf_exempt
def issue_action_menu(
    request, issue_id, template="timepiece/project/_issue_action_menu.html"
):
...
@csrf_exempt
def edit_client(
    request, client_code, template="timepiece/client/add_client.html", context=None
):
    context = context or {}
    if not request.user.is_superuser:
        return HttpResponseForbidden("Not allowed")
    ...
    form = timepiece_forms.ClientForm(
...
def project_cost_summary(
    request, project_id, template="timepiece/project/project_cost_summary.html"
):
    context = {}
    project = timepiece.Project.objects.get(pk=project_id)
    bp = timepiece.BusinessPermissions.for_user(request.user, project.business)
...
def issue_clock_in(request):
    ...
    for open_entry in (
        timepiece.Entry.objects.filter(user=request.user)
        .exclude(pk=new_entry.id)
        .is_open()
    ):
        open_entry.end_time = clock_time
...
def download_issue_attachment(request, issue_attachment_id):
    ...
    bp = timepiece.BusinessPermissions.for_user(
        request.user, issue_attachment.issue.project.business
    )
