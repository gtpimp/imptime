...
class MyLedgerTest(TimepieceDataTestCase):
    def setUp(self):
        ...
        self.url = reverse("view_person_time_sheet", kwargs={"user_id": self.user.pk})

    def login_with_permissions(self):
        view_entry_summary = Permission.objects.get(codename="view_entry_summary")
...
class ClockInTest(TimepieceDataTestCase):
    ...
    def testClockInAutoOut(self):
        ...
        self.assertEqual(
            closed_entry.end_time + datetime.timedelta(seconds=1),
    ...
    def testClockInSameTime(self):
        ...
        entry1 = self.create_entry(entry1_data)
        entry1_data.update({"st_str": self.now.strftime("%H:%M:%S")})
        data = self.clock_in_form
        data.update(
            {
    ...
    def testClockInBeforeCurrent(self):
        ...
        entry1 = self.create_entry(entry1_data)
        entry1_data.update({"st_str": self.ten_min_ago.strftime("%H:%M:%S")})
    ...
    def testClockInActiveTooLong(self):
        ...
        response = self.client.post(self.url, data)
        err_msg = (
            "Ending time exceeds starting time by 12 hours "
    ...
    def test_clock_in_active_comments(self):
        ...
        """
        entry = self.create_entry({"start_time": self.ten_min_ago})
...
class AutoActivityTest(TimepieceDataTestCase):
    ...
    def testLastWorkedSeveralEntries(self):
        ...
        for day in range(0, 10):
            ...
            self.log_time(start=this_day, project=self.project, activity=activity)
...
class ClockOutTest(TimepieceDataTestCase):
    ...
    def testClockOutReverse(self):
        ...
        self.assertFormError(
            response, "form", None, "Ending time must exceed the starting time"
        )
...
class CheckOverlap(TimepieceDataTestCase):
    ...
    def testOverlapWithPause(self):
        ...
        self.log_time(start=self.start_before, end=self.start_inside, pause=120)
        user_total_overlaps = self.use_checkoverlap(self.get_entries())
    ...
    def testOverlapWithoutEnoughPause(self):
        ...
        self.log_time(start=self.start_before, end=self.start_inside, pause=119)
        user_total_overlaps = self.use_checkoverlap(self.get_entries())
        self.assertEqual(user_total_overlaps, 1)
...
class CreateEditEntry(TimepieceDataTestCase):
    def setUp(self):
        ...
        self.edit_closed_url = reverse("timepiece-update", args=[self.closed_entry.pk])
        self.edit_current_url = reverse(
            "timepiece-update", args=[self.current_entry.pk]
        )
    ...
    def testEditClosed(self):
        ...
        """
        response = self.client.post(
            self.edit_closed_url, self.default_data, follow=True
        )
        self.assertRedirects(
            response,
            reverse("timepiece-entries"),
    ...
    def testEditCurrentDiffTime(self):
        ...
        """
        data = self.default_data
        new_start = self.current_entry_data["start_time"] + datetime.timedelta(
            minutes=5
        )
        data.update(
            {
    ...
    def add_entry_test_helper(self):
        ...
        response = self.client.post(
            self.create_url, data=self.default_data, follow=True
        )
        self.assertEqual(response.status_code, 200)

        msg = (
            "You cannot add entries after a timesheet has been approved "
...
class StatusTest(TimepieceDataTestCase):
    def setUp(self):
        ...
        self.sheet_url = reverse("view_person_time_sheet", args=[self.user.pk])
        self.verify_url = reverse(
            "change_person_time_sheet",
    ...
    def test_verify_other_user(self):
        ...
        url = reverse(
            "change_person_time_sheet",
            args=("verify", self.user2.pk, self.from_date.strftime("%Y-%m-%d")),
        )
    ...
    def test_approve_user(self):
        ...
        self.assertContains(
            response, "Forbidden: You cannot approve this timesheet", status_code=403
        )
...
class HourlySummaryTest(TimepieceDataTestCase):
    ...
    def test_single_entry_in_week(self):
        ...
        """
        april = utils.add_timezone(datetime.datetime(month=4, day=1, year=2012))
        march = utils.add_timezone(datetime.datetime(month=3, day=26, year=2012))
        self.create_entry(
            {
                "user": self.user,
                "start_time": april,
        ...
        response = self.client.get(
            self.url + "?{0}".format(urllib.urlencode({"year": 2012, "month": 4}))
        )
        self.assertEquals(response.status_code, 200)
        ...
        entries = timepiece.Entry.objects.timespan(april, span="month").date_trunc(
            "month", extra_values
        )
