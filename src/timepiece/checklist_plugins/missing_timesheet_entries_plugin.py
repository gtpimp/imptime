    ...
    def check_for_problems(self):
        ...
        for project in active_projects:
            ...
            if most_recent_entry is None:
                problems.append(self._create_problem_item(msg="In dev sprint %s has no timesheet entries"%(project), project=project))
            else:
                ...
                if num_days_since_last_entry > settings.EXPECTING_A_TIMESHEET_ENTRY_EVERY_X_DAYS:
                    problems.append(self._create_problem_item(msg="In dev sprint %s hasn't had a timesheet entry for %d days"%(project, num_days_since_last_entry), project=project))
