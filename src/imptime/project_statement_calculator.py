...
class ProjectStatementCalculator(object):

    def get_data(self, user, project_id, date_from_inclusive=None, date_to_inclusive=None, sprint_ids=None):
        project = Project.objects.get(pk=project_id)
        bp = BusinessPermissions.for_user(user, project)  # sic
        self.user = user
        if not bp.has_view_ctc_billable_rates:
            raise Exception("No permission to view project statement")
