...
class FeatureViewSet(BaseViewSet):
    ...
    def list(self, request):
        ...
        try:
            ...
            if project_id:
                features = features.order_by_project_id(
                    project_id=project_id, parent_feature_id=None
                )
            ...
            if format_args.get("ids_only", None):
                if not project_id:
                    ...
                    )  # for the moment, this is just a sanity check
                context["ids"] = [str(x) for x in features.values_list("id", flat=True)]
            else:
                if features.count() > 0:
    ...
    def _enrich_features_qs(self, features, project_id):
        features = (
            features.prefetch_related("children")
            ...
            for feature_pk in feature_pks:
                ...
                if field_name == "name":
                    ...
                    if self.logged_in_permissions(feature.project).has_edit_feature:
                        ...
                        feature.enriched_description = MarkdownEnrichment(
                            request.user
                        ).enrich(
                            feature.description, project_id=feature.project_id
                        )  # sic
                        FeatureHistory.add_history(
                            request.user,
                            feature,
                    ...
                    if self.logged_in_permissions(feature.project).has_edit_feature:
                        ...
                        if new_parent_id != feature.parent_id:
                            ...
                            FeatureHistory.add_history(
                                ...
                                feature,
                                "updated parent",
                                feature.parent.name if feature.parent else "root",
                                new_parent.name,
                            )
                        ...
                        if new_sibling_node_before_id is None:
                            FeatureHistory.add_history(
                                request.user, feature, "moved", None, "to top"
                            )
                            ProjectFeatureOrder.insert_at_the_beginning(feature)
                        else:
                            new_sibling_node_before = self.allowed_feature(
                                new_sibling_node_before_id
                            )
                            FeatureHistory.add_history(
                                request.user,
                                feature,
                                "moved",
                                None,
                                "after %s" % new_sibling_node_before.name,
                            )
                            ProjectFeatureOrder.insert_after(
                                feature, new_sibling_node_before
                            )
                    ...
                    if self.logged_in_permissions(feature.project).has_edit_features:
                        if new_value is None:
                            ...
                            ProjectFeatureOrder.insert_after(
                                feature, set_after_this_feature=after_feature
                            )
    ...
    def create(self, request):
        ...
        try:
            ...
            def create_feature():
                ...
                FeatureHistory.add_history(
                    request.user, feature, "created", "", feature.name
                )
                return feature

            feature = create_feature()
            feature = self._enrich_features_qs(
                Feature.objects.filter(pk=feature.id), project
            ).first()

            self._calculate_feature_stats(request.user, [feature])
            context["item"] = FeatureSerializer(
                feature, logged_in_user=request.user
            ).data
            data = {"status": "success", "payload": context}
    ...
    def bulk_create_features(self, request):
        try:
            ...
            new_features = BulkTextParser(request.user).create_features(
                raw_text=bulk_feature_text,
                project=project,
            ...
            data = {
                "status": "success",
                "payload": {"new_feature_ids": new_feature_ids},
            }
    ...
    def delete(self, request, pk):
        try:
            ...
            for feature_pk in feature_pks:
                ...
                if self.logged_in_permissions(feature.project).has_edit_feature:
                    FeatureHistory.add_history(
                        request.user, feature, "deleted", feature.name, ""
                    )
                    feature.delete()
                else:
                    data = {
                        "status": "failed",
                        "error_message": "Permission denied to delete features",
                    }
    ...
    def apply_filter(self, qs, raw_filter_args):
        ...
        if feature_any_field is not None and len(feature_any_field) > 1:
            qs = qs.filter(
                Q(name__icontains=feature_any_field)
                | Q(description__icontains=feature_any_field)
            )

        return super(FeatureViewSet, self).apply_filter(
    ...
    def _calculate_feature_stats(self, logged_in_user, features):
        ...
        for feature in features:
            self._recursively_calculate_nested_stats(
                logged_in_user, features_by_id, feature
            )

        return features

    @classmethod
    def _recursively_calculate_nested_stats(
        self, logged_in_user, features_by_id, feature
    ):
        ...
        for child_id in [x.id for x in feature.children.all()]:
            ...
            if not hasattr(child, "nested_stats"):
                self._recursively_calculate_nested_stats(
                    logged_in_user, features_by_id, child
                )
        ...
        for feature_testable in testables:
            ...
            for issue in issues:
                ...
                if not hasattr(self, "_cached_permissions"):
                    ...
                    self._cached_permissions = ProjectPermissions.for_user(
                        user=logged_in_user,
                ...
                points = [
                    x
                    for x in issue.issue_points.all()
                    if x.user_id == issue.assigned_to_id
                ]
                if len(points) == 0:
                    ...
                    if (
                        issue.assigned_to_id == logged_in_user.id
                        or self._cached_permissions.has_see_other_user_points
                    ):
