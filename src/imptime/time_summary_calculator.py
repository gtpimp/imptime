...
class TimeSummaryCalculator(object):

    def get_data(self, sprint_id, user):
        sprint = Sprint.objects.get(pk=sprint_id)

        bp = BusinessPermissions.for_user(user, sprint.business) #sic
        if not bp.has_view_ctc_billable_rates:
            raise Exception("No permission to view time summary")
        ...
        sprint_users = sprint.business.get_users_allowed_to_estimate_on_business(user)
        ...
        for sprint_developer in sprint_developers:
            dev_stats = self._calculate_dev_hours_stats(sprint, sprint_developer)
    ...
    def _calculate_dev_hours_stats(self, sprint, user):
