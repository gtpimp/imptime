...
from timepiece.models import Activity, Attribute
from timepiece.models import Business as Project
from timepiece.models import Entry, Issue, IssueComment, IssueStatus, Location
...
class Command(BaseCommand):
    ...
    def stop(self):
        ...
        try:
            ...
            logger.info(
                "Failed to kill existing process with pid %s, assuming already dead so ignoring"
                % pid
            )
            pass
        ...
        for message_number in raw_email_numbers[0].split():
            try:
                try:
                    ...
                    logger.exception(ex)
                    send_mail(
                        subject="Problems parsing email: %s" % message_number,
                        message=message,
                ...
                try:
                    ...
                    raw_issues = self.resolve_issue_content(
                        message, default_subject, project
                    )
                    for raw_issue in raw_issues:
                        new_issue = self.create_issue(
                            message, user, project, sprint, raw_issue
                        )
                        logger.debug(
                            "Created issue %s %s" % (new_issue.id, new_issue.subject)
                        )
                    ...
                    send_mail(
                        subject="Couldn't create issues from email",
                        message="Failed to process your email. Please resend it \n\n%s\n\n%s"
                        % (ex, str(email_message)),
                        from_email=settings.FROM_EMAIL,
                ...
                logger.exception(ex)
                send_mail(
                    subject="Issue creator general error: %s" % message_number,
                    message=str(ex),
                    from_email=settings.FROM_EMAIL,
                    recipient_list=settings.EMACS_ADMIN_USER_EMAILS,
                    fail_silently=True,
    ...
    def notify_issues_created(self, user, project, issues_created):
        ...
        if len(issues_created) == 1:
            subject = "Issue by email for %s: %s" % (
                project.name,
                issues_created[0].subject,
            )
        else:
    ...
    def resolve_parts(self, message):
        ...
        project = (
            Project.objects.filter(name__iexact=project_name)
        ...
        sprint = (
            Sprint.objects.filter(
                business=project, name=sprint_name, project_type="inbox"
            )
            .filter_open()
            .order_by("-id")
            .first()
        ...
        if sprint is None:
            sprint = Sprint.objects.get_or_create(
                ...
                name=sprint_name,
                defaults={
                    "status3": SprintStatus.objects.get_or_create(
                        name="pending", business=project
                    )[0],
                    "project_type": "inbox",
    ...
    def resolve_issue_content(self, message, default_subject, project):
        ...
        if "***" in content:
            ...
            raw_issues.append(
                {
                    "subject": default_subject or content[0:20],
                    "description": content,
                }
            )
        return raw_issues
    ...
    def create_issue(self, message, user, project, sprint, raw_issue):
        issue = (
            Issue.objects.filter(project=sprint, subject=raw_issue["subject"])
        ...
        if issue is None:
            issue = Issue.objects.create(
                ...
                status2=IssueStatus.objects.get_or_create(name="new", business=project)[
                    0
                ],
                assigned_to=user,
                number=Issue.get_next_issue_number(project),
                description=raw_issue["description"][
                    0 : settings.ISSUE_INBOX_MAX_ISSUE_DESCRIPTION_LENGTH
                ],
                story_points=0,
        ...
        for attachment_content in message["files"]:
            temp_physical_filename = os.path.join(
                settings.ISSUE_INBOX_TEMP_ATTACHMENT_FOLDER,
            ...
            VisualSpecDocument.create_for_doc(
                user=user,
                project=project,
                doc=django_file,
