...
class MailerMessage(models.Model):
    ...
    def add_attachment(self, attachment, name=None):
        """
        ...
        Attachment.objects.create(email=self, file_attachment=attachment, name=name)
    ...
    def _send(self):
        if not self.sent:
            ...
            for attachment in self.attachment_set.all():
                filename = attachment.file_attachment.file.name
                # if filename.endswith("csv"):
                    # logger.info("Hack")
                    # filename = os.path.join(settings.GENERAL_TEMP_FOLDER, filename.split("/")[-1])
                if attachment.name:
