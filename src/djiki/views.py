...
def view(request, title, revision_pk=None):
	...
	if title != url_title:
		if revision_pk:
			return HttpResponseRedirect(reverse('wiki:djiki-page-revision',
						kwargs={'title': url_title, 'revision_pk': revision_pk}))
		return HttpResponseRedirect(reverse('wiki:djiki-page-view', kwargs={'title': url_title}))
	...
	if revision_pk:
		...
		messages.info(request, mark_safe(_("The version you are viewing is not the latest one, "
				...
				"<a href=\"%(url)s\">proceed to the latest version</a>.") % {
					'url': reverse('wiki:djiki-page-view', kwargs={'title': url_title})}))
	else:
...
def edit(request, title):
	url_title = utils.urlize_title(title)
	if title != url_title:
		return HttpResponseRedirect(reverse('wiki:djiki-page-edit', kwargs={'title': url_title}))
	page_title = utils.deurlize_title(title)
	auth = utils.get_auth_backend()
	try:
		page = models.Page.objects.get(title=page_title)
	...
	if form.is_valid():
		if request.POST.get('action') == 'preview':
			...
			return HttpResponseRedirect(
					reverse('wiki:djiki-page-view', kwargs={'title': url_title}))
...
def history(request, title):
	url_title = utils.urlize_title(title)
	if title != url_title:
		return HttpResponseRedirect(reverse('wiki:djiki-page-history', kwargs={'title': url_title}))
	page_title = utils.deurlize_title(title)
...
def revert(request, title, revision_pk):
	...
	if title != url_title:
		return HttpResponseRedirect(
				reverse('wiki:djiki-page-revert', kwargs={'title': url_title, 'revision_pk': revision_pk}))
	page_title = utils.deurlize_title(title)
	page = get_object_or_404(models.Page, title=page_title)
	auth = utils.get_auth_backend()
	if not auth.can_edit(request, page):
...
def undo(request, title, revision_pk):
	...
	if title != url_title:
		return HttpResponseRedirect(
				reverse('wiki:djiki-page-undo', kwargs={'title': url_title, 'revision_pk': revision_pk}))
	page_title = utils.deurlize_title(title)
	page = get_object_or_404(models.Page, title=page_title)
	auth = utils.get_auth_backend()
	if not auth.can_edit(request, page):
	...
	if request.method == 'POST':
		...
		if False in results:
			...
			return HttpResponseRedirect("%s?%s" % (
					reverse('wiki:djiki-page-diff', kwargs={'title': url_title}),
					urlencode(urldata)))
...
def image_new(request):
	...
	if request.method == 'POST':
		if form.is_valid():
			form.save()
			return HttpResponseRedirect(
					reverse('wiki:djiki-image-view', kwargs={'name': form.instance.image.name}))
...
def image_view(request, name):
	...
	if name != url_name:
		return HttpResponseRedirect(reverse('wiki:djiki-image-view', kwargs={'name': url_name}))
...
def image_edit(request, name):
	url_name = utils.urlize_title(name)
	if name != url_name:
		return HttpResponseRedirect(reverse('wiki:djiki-image-edit', kwargs={'name': url_name}))
	image_name = utils.deurlize_title(name)
	image = get_object_or_404(models.Image, name=image_name)
	auth = utils.get_auth_backend()
	...
	if request.method == 'POST':
		if form.is_valid():
			form.save()
			return HttpResponseRedirect(
					reverse('wiki:djiki-image-view', kwargs={'name': url_name}))
	return _templating.render_to_response(request, 'djiki/image_edit.html', {'form': form})
