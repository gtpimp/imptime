...
class SprintReconToolbarPanel extends Component {

    onPrint = (evt) => {
        const { dispatch, sprint } = this.props
        evt.preventDefault()
        dispatch(printCurrentPage(`Recon_${sprint.name}`))
    }
...
function mapStateToProps(state, props) {
    const page_key = PAGE_KEY__SPRINT_RECON_PAGE
...
export default withRouter(connect(mapStateToProps)(SprintReconToolbarPanel))
